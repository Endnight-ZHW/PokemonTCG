"""Deterministic Challenge-teacher generation for Deep AI v3.

The teacher deliberately uses the public Python rules facade because Challenge
AI is a Python policy.  Replay projection and every training contract are v3;
this module has no dependency on the retired AlphaZero implementation.
"""
...
from engine.native_state_codec import state_to_native_snapshot
...
def setup_teacher_game(task: TeacherTaskV3) -> GameState:
    ...
    with rng.bind_state(state):
        finish_setup(state, None, [_SetupAI(), _SetupAI()], rng)
...
def _challenge_ai(
    ...
    return create_challenge_ai(
        deck_key,
        AIConfig(deck_key=deck_key, random_seed=seed, **config),
    )
...
def _authoritative_action(
    ...
    promotions = [row for row in actions if row.kind_name == "PROMOTE"]
    if promotions and len(promotions) == len(actions):
        player = state.get_player(actor)

        def promotion_value(action: GameAction) -> float:
            index = action.bench_index()
            if (
                index < 0
                or index >= len(player.bench)
                or player.bench[index] is None
            ):
                return float("-inf")
            scorer = getattr(ai, "_forced_promotion_value", None)
            return (
                float(scorer(state, actor, player.bench[index]))
                if callable(scorer)
                else 0.0
            )

        return max(promotions, key=promotion_value), None

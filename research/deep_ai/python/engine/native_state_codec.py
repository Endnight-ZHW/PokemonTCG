...
# This adapter is private to research/deep_ai/python, four levels below the
# repository root. Product card data remains the shared, read-only catalog.
...
def native_catalog_payload() -> dict[str, Any]:
    ...
    card_ir = json.loads(
        (REPO_ROOT / "godot" / "data" / "card_ir_v4.json").read_text(
            encoding="utf-8"
        )
    )

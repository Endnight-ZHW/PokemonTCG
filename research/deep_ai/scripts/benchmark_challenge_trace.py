...
def main():
    ...
    for role in ("baseline", "candidate"):
        ...
        parser.add_argument(f"--{role}-prune-transfer-cycles", choices=("enabled", "disabled"))
    ...
    overrides = {role: {f"internal_{feature}": getattr(args, f"{role}_{feature}") == "enabled"
        for feature in ("anytime_search", "search_memoization", "dual_guidance", "prune_transfer_cycles")

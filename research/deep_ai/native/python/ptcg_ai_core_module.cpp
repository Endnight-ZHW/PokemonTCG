...
PYBIND11_MODULE(ptcg_ai_core, module) {
    ...
    py::class_<ChallengeController>(module, "ChallengeController")
        .def(py::init<>())
        .def(
            "configure",
            [](ChallengeController &controller,
               const py::dict &catalog,
               const py::dict &decks,
               const py::dict &strategies) {
                return value_to_python(controller.configure(
                    value_from_python(catalog),
                    value_from_python(decks),
                    value_from_python(strategies)));
            },
            py::arg("catalog"),
            py::arg("decks"),
            py::arg("strategies")
        ...
        .def(
            ...
            [](ChallengeController &controller,
               ...
               std::int64_t generation) {
                ...
                {
                    py::gil_scoped_release release;
                    result = controller.decide(native_request, generation);
                }
        ...
        .def("cancel", &ChallengeController::cancel, py::arg("generation"))
        .def("reset_match", &ChallengeController::reset_match,
             py::arg("match_instance_id"))
        .def("get_contract", [](const ChallengeController &controller) {
            return value_to_python(controller.get_contract());
        });

...
if ($mergeOnly) {
    ...
    # The stopwatch starts before the fixed 280-game preflight. Checkpoints
    # created by that preflight therefore remain part of this invocation's full
    # evidence-stage wall time; only files that existed before the stopwatch can
    # make the measurement a current-attempt-only value.
    ...
    function Get-PostPreflightShardOrder {
        param(
        ...
        $mirrorElapsedMs = 0.0
        $crossElapsedMsByGroup = @{}
        foreach ($path in $PreflightPaths) {
            $payload = Get-Content -Raw -LiteralPath $path | ConvertFrom-Json
            $evidenceShardIndex = [int]$payload.config.evidence_shard_index
            $group = [int][Math]::Floor($evidenceShardIndex / 10)
            foreach ($match in @($payload.matches)) {
                if ([string]$match.matchup_kind -eq 'mirror') {
                    $mirrorElapsedMs += [double]$match.elapsed_ms
                }
                elseif ([string]$match.matchup_kind -eq 'cross') {
                    $crossElapsedMsByGroup[$group] = (
                        [double]$crossElapsedMsByGroup[$group] +
                        [double]$match.elapsed_ms
                    )
                }
            }
        }

        $validCheckpointUnitsByShard = @{}
        if ($checkpointEnabled) {
            $provenancePayload = (
                Get-Content -Raw -LiteralPath $provenancePath |
                    ConvertFrom-Json
            )
            $simulationFingerprint = [string](
                $provenancePayload.simulation_fingerprint
            )
            if ($simulationFingerprint -notmatch '^[0-9a-f]{64}$') {
                throw (
                    'Cannot inspect evaluation checkpoints: provenance has ' +
                    'an invalid simulation_fingerprint.'
                )
            }
            $checkpointInspectionPath = Join-Path (
                $OutputDir
            ) 'checkpoint_inspection.json'
            $checkpointInspectionOutput = & $python @(
                (
                    Join-Path $repoRoot (
                        'python\scripts\inspect_ai_evaluation_checkpoints.py'
                    )
                ),
                '--checkpoint-root', $CheckpointDir,
                '--simulation-fingerprint', $simulationFingerprint,
                '--task-manifest-id', $taskManifestId,
                '--shard-count', ([string]$LogicalShardCount),
                '--output', $checkpointInspectionPath
            ) 2>&1
            $checkpointInspectionExitCode = $LASTEXITCODE
            $checkpointInspectionOutput | ForEach-Object { Write-Host $_ }
            if (
                $checkpointInspectionExitCode -ne 0 -or
                -not (Test-Path -LiteralPath $checkpointInspectionPath)
            ) {
                throw 'Failed to inspect resumable evaluation checkpoints.'
            }
            $checkpointInspection = (
                Get-Content -Raw -LiteralPath $checkpointInspectionPath |
                    ConvertFrom-Json
            )
            if (
                [int]$checkpointInspection.schema_version -ne 7 -or
                [string]$checkpointInspection.protocol_id -ne (
                    'traditional_ai_evaluation_v7'
                ) -or
                [string]$checkpointInspection.artifact_kind -ne (
                    'ai_evaluation_checkpoint_inspection'
                ) -or
                [string]$checkpointInspection.simulation_fingerprint -ne (
                    $simulationFingerprint
                ) -or
                [string]$checkpointInspection.task_manifest_id -ne (
                    $taskManifestId
                ) -or
                [int]$checkpointInspection.evidence_shard_count -ne (
                    $LogicalShardCount
                )
            ) {
                throw 'Checkpoint inspector returned an incompatible artifact.'
            }
            $validUnitsObject = $checkpointInspection.valid_unit_ids_by_shard
            $validUnitsProperties = $validUnitsObject.PSObject.Properties
            foreach ($index in $EvidenceShards) {
                $shardProperty = @(
                    $validUnitsProperties |
                        Where-Object { $_.Name -eq [string]$index }
                ) | Select-Object -First 1
                if ($null -eq $shardProperty) {
                    throw (
                        'Checkpoint inspector omitted logical shard {0}.' -f
                        $index
                    )
                }
                $unitSet = [System.Collections.Generic.HashSet[string]]::new(
                    [System.StringComparer]::Ordinal
                )
                foreach ($unitId in @($shardProperty.Value)) {
                    [void]$unitSet.Add([string]$unitId)
                }
                $validCheckpointUnitsByShard[[int]$index] = $unitSet
            }
        }

        function Test-CheckpointUnitPresent {
            param(
                [int]$EvidenceShard,
                [string]$UnitId
            )
            ...
            if (-not $validCheckpointUnitsByShard.ContainsKey($EvidenceShard)) {
                return $false
            }
            return $validCheckpointUnitsByShard[$EvidenceShard].Contains($UnitId)
        }

        # The prefix measures one complete mirror-block cost and every cross
        # matchup group. Derive the remaining fraction from immutable checkpoint
        # identities so a later resume schedules genuinely expensive shards first,
        # not merely the fixed prefix that happened to run in this attempt.
        $mirrorBlockElapsedMs = $mirrorElapsedMs / 5.0
        $remainingCostByShard = @{}
        foreach ($index in $EvidenceShards) {
            $group = [int][Math]::Floor($index / 10)
            $completedMirrorUnits = 0
            for ($deckIndex = 0; $deckIndex -lt $defaultDeckKeys.Count; $deckIndex++) {
                ...
                $unitId = 'mirror|{0}|{1}|{2}' -f @(
                    $defaultDeckKeys[$deckIndex],
                    $blockIndex,
                    $gameSeed
                )
                if (Test-CheckpointUnitPresent -EvidenceShard $index -UnitId $unitId) {
                    $completedMirrorUnits += 1
                }
            }

            $completedCrossUnits = 0
            $pairOrdinal = 0
            for ($lower = 0; $lower -lt $defaultDeckKeys.Count - 1; $lower++) {
                for ($upper = $lower + 1; $upper -lt $defaultDeckKeys.Count; $upper++) {
                    for ($blockIndex = 0; $blockIndex -lt $effectiveCrossSeedBlocks; $blockIndex++) {
                        $unitIndex = (
                            $defaultDeckKeys.Count * $SeedBlocksPerDeck +
                            $pairOrdinal * $effectiveCrossSeedBlocks +
                            $blockIndex
                        )
                        if (($unitIndex % $LogicalShardCount) -ne $index) {
                            continue
                        }
                        $gameSeed = (
                            $Seed +
                        ...
                        $unitId = 'cross|{0}|{1}|{2}|{3}' -f @(
                            $defaultDeckKeys[$lower],
                            $defaultDeckKeys[$upper],
                            $blockIndex,
                            $gameSeed
                        )
                        if (
                            Test-CheckpointUnitPresent `
                                -EvidenceShard $index `
                                -UnitId $unitId
                        ) {
                            $completedCrossUnits += 1
                        }
                    }
                    $pairOrdinal += 1
                }
            ...
            $remainingMirrorFraction = (
                [Math]::Max(0, $defaultDeckKeys.Count - $completedMirrorUnits) /
                [double]$defaultDeckKeys.Count
            )
            $crossUnitsPerShard = [int](
                ($defaultDeckKeys.Count * ($defaultDeckKeys.Count - 1) / 2) *
                $effectiveCrossSeedBlocks /
                $LogicalShardCount
            )
            $remainingCrossFraction = if ($crossUnitsPerShard -gt 0) {
                [Math]::Max(0, $crossUnitsPerShard - $completedCrossUnits) /
                    [double]$crossUnitsPerShard
            }
            else {
                0.0
            }
            $remainingCostByShard[[int]$index] = (
                $mirrorBlockElapsedMs * $remainingMirrorFraction +
                [double]$crossElapsedMsByGroup[$group] * $remainingCrossFraction
            )
        }
        return @(
            $EvidenceShards |
                Sort-Object `
                    @{ Expression = { -[double]$remainingCostByShard[[int]$_] } }, `
                    @{ Expression = { [int]$_ } }
        )
    }
    ...
    $isFullLocalNightly = (
        $EvalPreset -eq 'Nightly' -and
        $ShardCount -eq 0 -and
        $LogicalShardCount -eq 50 -and
        $selectedDeckKeys.Count -eq $defaultDeckKeys.Count -and
        $effectiveMatchupMode -eq 'Balanced' -and
        $SeedBlocksPerDeck -eq 50 -and
        $effectiveCrossSeedBlocks -eq 10 -and
        $SeedBlockStart -eq 0 -and
        $SeedBlockCount -eq 0 -and
        $TaskStart -eq 0 -and
        $TaskCount -eq 0
    )
    if ($isFullLocalNightly -and $checkpointEnabled -and -not $NoResume -and -not $SkipPreflight) {
        Write-Host 'Running the fixed 280-game structural preflight before the full matrix.'
        $preflightArgs = @($runnerArgs)
        if ('--fail-fast-fatal' -notin $preflightArgs) {
            $preflightArgs += @('--fail-fast-fatal')
        }
        $preflightArgs += @('--evidence-prefix-only')
        $preflightEvidenceShards = @(
            $assignedEvidenceShards |
                Where-Object { $_ -in @(0, 1, 2, 3, 4, 10, 20, 30, 40) }
        )
        $preflightJsonPaths = @(Invoke-EvidenceShardStage `
            -BaseArgs $preflightArgs `
            -StageRoot (Join-Path $OutputDir 'preflight\shards') `
            -StageLabel 'preflight' `
            -ForceSkipGolden $false `
            -EvidenceShards $preflightEvidenceShards)
        $preflightGoldenChecked = $false
        foreach ($preflightPath in $preflightJsonPaths) {
            $preflightPayload = Get-Content -Raw -LiteralPath $preflightPath |
                ConvertFrom-Json
            if ([int]$preflightPayload.config.evidence_shard_index -ne 0) {
                continue
            }
            $preflightGoldenChecked = $true
            if (
                [int]$preflightPayload.golden_scenarios.total -lt 1 -or
                [int]$preflightPayload.golden_scenarios.failed -gt 0
            ) {
                throw (
                    'The fixed 280-game preflight failed its tactical golden checks: ' +
                    ($preflightPayload.golden_scenarios | ConvertTo-Json -Compress -Depth 8)
                )
            }
        }
        if (-not $preflightGoldenChecked) {
            throw 'The fixed 280-game preflight did not produce its tactical golden evidence.'
        }
        $assignedEvidenceShards = @(
            Get-PostPreflightShardOrder `
                -PreflightPaths $preflightJsonPaths `
                -EvidenceShards $assignedEvidenceShards
        )
        Write-Host (
            'Main evidence shards reordered by deterministic preflight LPT weights: ' +
            ($assignedEvidenceShards -join ',')
        )
    }

...
Invoke-GodotCheckedScript `
    -Script 'res://tests/native_game_golden_test.gd' `
    -SuccessMarker 'NATIVE_GAME_GOLDEN_OK' `
    -ContractName '23-case C++ native game action semantic golden'
    ...
    $evidence = [ordered]@{
        ...
        hashes = [ordered]@{
            ...
            native_game_source_sha256 = (
                Get-FileHash -LiteralPath (
                    Join-Path $repoRoot (
                        'godot\native\onnx_ai\src\ptcg_game.cpp'
                    )
                ) -Algorithm SHA256
            ).Hash.ToLowerInvariant()
        }

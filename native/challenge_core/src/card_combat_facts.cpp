...
std::vector<const Value *> board(const Value &state, std::int32_t actor) {
    ...
    if (active_value != nullptr && active_value->is_object())
        result.push_back(active_value);
...
bool had_knockout(const Value &state, std::int32_t defeated, bool attack_only) {
    ...
    for (const Value &fact : facts->as_array()) {
        if (integer_field(fact, "defeated_player", -1) != defeated ||
            integer_field(fact, "source_player", -1) < 0)
            continue;
        if (!attack_only)
            return true;
        if (integer_field(fact, "source_player", defeated) != defeated &&
            string_field(fact, "source_kind") == "attack_damage" &&
            string_field(fact, "cause_kind") == "damage")
            return true;
    }
    return false;
}

void append_flattened_effect(const Value &effect, std::vector<const Value *> &result) {
    if (!effect.is_object())
        return;
...
std::vector<const Value *> flatten_effects(const Value::Array &effects) {
    ...
    for (const Value &effect : effects)
        append_flattened_effect(effect, result);
    return result;
}
...
bool branch_has_effect(const Value *branch, const std::string &kind) {
    ...
    Value::Array effects;
    if (branch->is_object())
        effects.push_back(*branch);
    else if (branch->is_array())
        effects = branch->as_array();
    for (const Value *effect : flatten_effects(effects)) {
        if (string_field(*effect, "effect_type") == kind)
            return true;
    }
    return false;
}
...
bool condition_applies(const Value &state, std::int32_t actor, const Value &params,
    ...
    if (condition == "field_energy_ge_5") {
        ...
        for (const Value *pokemon : board(state, actor))
            count += energy_unit_count(cards, pokemon);
        return count >= 5;
    }
    ...
    if (condition == "ko_last_opponent_turn")
        return had_knockout(state, actor, false);
    return opponent_active != nullptr && integer_field(*opponent_active, "damage_counters") > 0;
}

std::int64_t branch_damage(const RulesSession &position, const Value &state, std::int32_t actor,
                           const Value *branch, const Value &cards) {
    if (branch == nullptr)
        return 0;
    Value::Array effects;
    if (branch->is_object())
...
std::int64_t effect_damage(const RulesSession &position, const Value &state, std::int32_t actor,
                           const Value &effect, const Value &cards) {
    ...
    if (kind == "damage")
        return integer_field(params, "amount");
    if (kind == "conditional_damage_bonus")
        return condition_applies(state, actor, params, cards)
                   ? integer_field(params, "bonus", integer_field(params, "amount"))
                   : 0;
    if (kind == "damage_per_self_damage")
    ...
    if (kind == "damage_per_self_energy" || kind == "damage_per_self_energy_type") {
        const std::string filter =
            lower_ascii(string_field(params, "energy_filter", string_field(params, "energy_type")));
        return integer_field(params, "base") +
    ...
    if (kind == "damage_per_energy") {
        ...
        if (from == "opponent_active")
            count = energy_unit_count(cards, opponent_active);
        else if (from == "all_opponent") {
            for (const Value *pokemon : board(state, 1 - actor))
                count += energy_unit_count(cards, pokemon);
        } else
            count = energy_unit_count(cards, own_active);
    ...
    if (kind == "attack_damage_formula") {
        ...
        if (own_active != nullptr)
            total += integer_field(*own_active, "damage_counters") *
        ...
        if (bonus != nullptr && bonus->is_object()) {
            ...
            if (condition == "ko_by_attack_last_turn" ||
            ...
            else if (condition == "ko_last_opponent_turn")
                applies = had_knockout(state, actor, false);
            else if (condition == "own_bench_damaged") {
                applies = std::any_of(array_field(own, "bench").begin(),
                                      array_field(own, "bench").end(), [](const Value &pokemon) {
                                          return pokemon.is_object() &&
                                                 integer_field(pokemon, "damage_counters") > 0;
                                      });
                ...
                applies = opponent_active != nullptr &&
                          integer_field(*opponent_active, "damage_counters") > 0;
            } else if (condition == "own_hand_empty")
                applies = array_field(own, "hand").empty();
            if (applies)
                total += integer_field(*bonus, "bonus");
        }
        return total;
    }
    if (kind == "damage_per_hand_size")
        return static_cast<std::int64_t>(array_field(own, "hand").size()) *
    ...
    if (kind == "discard_hand_conditional_bonus") {
        ...
        return base + (array_field(own, "hand").size() >=
                               static_cast<std::size_t>(
                                   std::max<std::int64_t>(0, integer_field(params, "threshold", 5)))
                           ? integer_field(params, "bonus")
                           : 0);
    }
    if (kind == "damage_per_discard_psychic") {
        ...
        for (const Value &entry : array_field(own, "discard")) {
            ...
            if (definition != nullptr && is_pokemon(cards, entry.string_or()) &&
                array_contains(array_field(*definition, "energy_types"), "Psychic"))
                ++count;
        }
        ...
        if (own_active != nullptr) {
            ...
            for (std::size_t index = 0; index < attached.size(); ++index) {
                ...
                if (std::find(units.begin(), units.end(), "Fighting") != units.end() ||
                    std::find(units.begin(), units.end(), "Rainbow") != units.end())
                    ++count;
            }
        }
    ...
    if (kind == "damage_per_evolved") {
        ...
        for (const Value *pokemon : board(state, actor)) {
            if (!array_field(*pokemon, "evolution_stack_ids").empty())
                ++evolved;
        }
        return evolved * integer_field(params, "per_evolved");
    }
    if (kind == "conditional_damage_heal")
        return integer_field(params, "base") +
               (own_active != nullptr && bool_field(*own_active, "healed_this_turn")
                    ? integer_field(params, "bonus")
                    : 0);
    if (kind == "damage_and_self_heal")
        return integer_field(params, "damage", integer_field(params, "amount"));
    if (kind == "any_pokemon_damage" || kind == "bench_damage")
        return integer_field(params, "amount", integer_field(params, "damage"));
    if (kind == "place_counters_and_self_discard")
        return integer_field(
            params, "amount",
            integer_field(params, "damage", integer_field(params, "counters") * 10));
    if (kind == "mill_and_damage_per_energy") {
        const auto &deck = array_field(own, "deck");
        const std::size_t count = static_cast<std::size_t>(
            std::max<std::int64_t>(0, integer_field(params, "mill_count", 5)));
        ...
        for (std::size_t index = begin; index < deck.size(); ++index) {
            if (is_energy(cards, deck[index].string_or()))
                ++energy_seen;
        }
        return energy_seen * integer_field(params, "damage_per");
    }
    if (kind == "damage_self_penalty")
        return std::max<std::int64_t>(
            0, integer_field(params, "base") -
                   (own_active == nullptr ? 0 : integer_field(*own_active, "damage_counters")) *
                       integer_field(params, "per_counter"));
    if (kind == "coin_flip_triple")
        return static_cast<std::int64_t>(
            static_cast<double>(integer_field(params, "damage_per_head", 10)) * 1.5);
    if (kind == "coin_flip_until_tails")
        return integer_field(params, "per_head", 20);
    if (kind == "coin_flip_double_ko")
        return static_cast<std::int64_t>(
            static_cast<double>(
                opponent_active == nullptr ? 0 : position.pokemon_current_hp(*opponent_active)) *
            0.25);
    if (kind == "coin_flip")
        return static_cast<std::int64_t>(
            static_cast<double>(
                branch_damage(position, state, actor, field(params, "on_heads"), cards) +
                branch_damage(position, state, actor, field(params, "on_tails"), cards)) *
            0.5);
    return 0;
}

std::int64_t reference_modified_attack_damage(const Value &state, std::int32_t actor,
                                              const Value &attacker, const Value &defender,
                                              std::int64_t base_damage, const Value &cards,
                                              bool ignore_defender_damage_effects,
                                              bool ignore_weakness, bool ignore_resistance) {
    if (base_damage <= 0)
        return std::max<std::int64_t>(0, base_damage);
    std::int64_t damage = base_damage;
    const Value *attacker_definition = card(cards, string_field(attacker, "card_id"));
    const Value *defender_definition = card(cards, string_field(defender, "card_id"));

    for (const Value *aura_source : board(state, actor)) {
        const Value *source_definition = card(cards, string_field(*aura_source, "card_id"));
        ...
        for (const Value &ability : array_field(*source_definition, "abilities")) {
            for (const Value &effect : array_field(ability, "effects")) {
                ...
                const Value &params =
                    params_value != nullptr && params_value->is_object() ? *params_value : empty;
                const std::string attacker_subtype = string_field(params, "attacker_subtype");
                const std::string defender_type = string_field(params, "defender_type");
                if (!attacker_subtype.empty() &&
                    (attacker_definition == nullptr ||
                     !array_contains(array_field(*attacker_definition, "subtypes"),
                                     attacker_subtype)))
                    continue;
                if (!defender_type.empty() &&
                    (defender_definition == nullptr ||
                     !array_contains(array_field(*defender_definition, "energy_types"),
                                     defender_type)))
                    continue;
    ...
    for (const Value &energy_id : array_field(attacker, "energy_card_ids")) {
        ...
        for (const Value &effect : array_field(*energy, "energy_effects")) {
            ...
            const Value *delta = operation != nullptr && operation->is_object()
                                     ? field(*operation, "delta")
                                     : nullptr;
            if (string_field(effect, "kind") == "modifier" &&
                string_field(effect, "hook") == "MODIFY_DAMAGE" &&
                string_field(effect, "scope") == "attached_attacker" && delta != nullptr &&
                delta->is_integer())
    ...
    for (const Value &modifier : array_field(attacker, "modifiers")) {
        const Value *operation = field(modifier, "operation");
        if (string_field(modifier, "hook") == "MODIFY_DAMAGE" &&
            string_field(modifier, "scope") == "attached_attacker" && operation != nullptr &&
            operation->is_object() && string_field(*operation, "kind") == "damage_delta" &&
    ...
    if (tool != nullptr && !tool_boost_materialized) {
        for (const Value &effect : array_field(*tool, "trainer_effects")) {
            if (string_field(effect, "effect_type") != "tool")
                continue;
            const Value *params_value = field(effect, "params");
            static const Value empty = Value::make_object();
            const Value &params =
    ...
    const auto aura_reduction = [&](bool before_weakness) {
        ...
        if (defender_definition == nullptr)
            return reduction;
        for (const Value &ability : array_field(*defender_definition, "abilities")) {
            for (const Value &effect : array_field(ability, "effects")) {
                if (string_field(effect, "effect_type") != "aura_damage_reduction")
                    continue;
                const Value *params_value = field(effect, "params");
                static const Value empty = Value::make_object();
                const Value &params =
    ...
    if (!ignore_defender_damage_effects)
        damage -= aura_reduction(true);
    if (damage <= 0)
        return 0;
    const Value *card_rows = field(cards, "cards");
    damage = game_detail::apply_active_type_matchups(card_rows == nullptr ? cards : *card_rows,
                                                     state, attacker, defender, damage,
                                                     ignore_weakness, ignore_resistance);
    if (!ignore_defender_damage_effects) {
        damage -= aura_reduction(false);
        const std::string defender_tool_id = string_field(defender, "attached_tool_id");
        const Value *defender_tool =
            defender_tool_id.empty() ? nullptr : card(cards, defender_tool_id);
        if (defender_tool != nullptr) {
            for (const Value &effect : array_field(*defender_tool, "trainer_effects")) {
                if (string_field(effect, "effect_type") != "tool")
                    continue;
                const Value *params_value = field(effect, "params");
                static const Value empty = Value::make_object();
                const Value &params =
                    params_value != nullptr && params_value->is_object() ? *params_value : empty;
                if (string_field(params, "effect") == "damage_reduction_stage1" &&
        ...
        if (modifier_kind(&defender, "prevent_damage") ||
            modifier_kind(&defender, "prevent_effects"))
            return 0;
    }
    return std::max<std::int64_t>(0, damage);
}

std::int64_t estimated_attack_damage(const RulesSession &position, const Value &state,
                                     std::int32_t actor, std::size_t attack_index,
                                     const Value &cards) {
    ...
    for (const Value *effect : flattened) {
        ...
        if (kind == "coin_flip") {
            ...
            if (params != nullptr && branch_has_effect(field(*params, "on_tails"), "attack_fail")) {
                damage = static_cast<std::int64_t>(std::llround(
                    static_cast<double>(damage + branch_damage(position, state, actor,
                                                               field(*params, "on_heads"), cards)) *
                    0.5));
                continue;
            }
        }
        ...
        if (kind == "conditional_damage_bonus")
            ...
            damage = std::max(damage, estimate);
    }
    if (damage <= 0 || defender == nullptr)
        return std::max<std::int64_t>(0, damage);
    return reference_modified_attack_damage(state, actor, *attacker, *defender, damage, cards,
                                            ignore_defender, ignore_weakness, ignore_resistance);
}

std::int64_t best_available_damage(const RulesSession &position, const Value &state,
...
std::int64_t best_ready_damage_for_pokemon(const RulesSession &position, const Value &state,
                                           std::int32_t actor, const Value &pokemon,
                                           const std::string &slot, const Value &cards) {
    ...
    Value original_active = field(owner, "active") == nullptr ? Value() : *field(owner, "active");
    if (slot.rfind("bench_", 0) == 0) {
        ...
        if (bench == nullptr || !bench->is_array() || bench_index < 0 ||
    ...
    for (std::size_t index = 0; index < attacks.size(); ++index) {
        if (missing_energy(cards, pokemon, array_field(attacks[index], "cost")) <= 0) {
            best =
                std::max(best, estimated_attack_damage(position, simulation, actor, index, cards));
        }
    }
    return best;
}
            ...
            if (kind == "damage_per_self_damage")
                ...
                damage = std::max(damage, integer_field(params, "base") +
                                              energy_type_count(cards, pokemon, type) *
                                                  integer_field(params, "per_energy"));
            } else if (kind == "damage_plus_bench")
                damage = std::max(damage, integer_field(params, "base") +
                                              integer_field(params, "per_bench") * 3);
            else if (kind == "discard_fighting_energy_damage")
                damage = std::max(damage, integer_field(params, "base") +
                                              energy_type_count(cards, pokemon, "Fighting") *
                                                  integer_field(params, "per_energy"));
            else if (kind == "damage_self_penalty")
                damage = std::max<std::int64_t>(0, integer_field(params, "base") -
                                                       integer_field(*pokemon, "damage_counters") *
                                                           integer_field(params, "per_counter"));
            else if (kind == "conditional_damage_bonus")
                damage += integer_field(params, "bonus", integer_field(params, "amount"));
            else if (kind == "attack_damage_formula") {
                std::int64_t formula =
                    integer_field(params, "base") + integer_field(params, "per_own_bench") * 3;
                const std::string type = string_field(params, "per_self_energy_type");
                if (!type.empty())
                    formula += energy_type_count(cards, pokemon, type) *
                               integer_field(params, "per_energy");
                const Value *bonus = field(params, "condition_bonus");
                if (bonus != nullptr)
                    formula += integer_field(*bonus, "bonus");
...
std::int64_t action_strength_damage(const Value &cards, const Value &definition,
                                    const Value *pokemon, std::int64_t energy_count) {
    std::int64_t best = 0;
        ...
        for (const Value *effect : flatten_effects(array_field(attack, "effects"))) {
            ...
            if (kind == "damage_plus_bench") {
                ...
                const std::int64_t units =
                    pokemon == nullptr ? energy_count : energy_type_count(cards, pokemon, type);
                damage = std::max(damage, integer_field(params, "base") +
                                              units * integer_field(params, "per_energy"));
            } else if (kind == "damage_self_penalty") {
                const std::int64_t counters =
                    pokemon == nullptr ? 0 : integer_field(*pokemon, "damage_counters");
                damage =
                    std::max<std::int64_t>(0, integer_field(params, "base") -
...
bool modifier_kind(const Value *pokemon, const std::string &kind, const std::string &text) {
    ...
    for (const Value &modifier : array_field(*pokemon, "modifiers")) {
        ...
        const std::string value =
            string_field(*operation, "attack_name", string_field(*operation, "reason"));
        if (value == text || value == "__all__")
            return true;
    }
    return false;
}
...
double deck_pressure(const Value &owner) {
    ...
    if (size == 0)
        return 520.0;
    if (size <= 2)
        return 135.0 + static_cast<double>(3 - size) * 65.0;
    if (size <= 5)
        return 70.0 + static_cast<double>(6 - size) * 12.0;
    if (size <= 8)
        return 38.0 + static_cast<double>(9 - size) * 5.0;
    if (size <= 12)
        return 18.0;
    return 0.0;
}
...
std::string deck_key(const Value &state, std::int32_t actor) {
    ...
    return actor >= 0 && static_cast<std::size_t>(actor) < keys.size()
               ? keys[static_cast<std::size_t>(actor)].string_or()
               : std::string{};
}

bool energy_matches_profile(const Value &cards, const std::string &card_id,
...
std::int64_t energy_profile_match_count(const Value &cards, const std::string &card_id,
                                        const std::string &key) {
    const Value *definition = card(cards, card_id);
    if (definition == nullptr)
        return 0;
    const auto &provided = array_field(*definition, "provides_energy");
    std::int64_t count = 0;
    // Rainbow satisfies each listed energy type in the profile.
    for (const Value &type_value : profile(cards, key).cards("energy")) {
        const std::string type = type_value.string_or();
        if (array_contains(provided, type) || array_contains(provided, "Rainbow")) {
            ++count;
...
double card_priority(const Value &cards, const std::string &card_id, const std::string &key) {
    ...
    double score = 0.0;
    if (p.contains("core", card_id))
        score += 180.0;
    if (p.contains("engine", card_id))
        score += 100.0;
    if (p.contains("evolution", card_id))
        score += 120.0;
    if (p.contains("trainer", card_id))
        score += 60.0;
    score += static_cast<double>(energy_profile_match_count(cards, card_id, key)) * 50.0;
    return score;
}

Value pokemon_with_extra_energy(const Value &pokemon, const std::string &energy_card_id) {
...
std::int64_t best_missing_with_extra(const Value &cards, const Value *pokemon,
...
bool place_candidate_in_active(Value &simulation, std::int32_t actor, const Value &pokemon,
...
std::int64_t estimated_damage_for_pokemon(const RulesSession &position, const Value &state,
                                          std::int32_t actor, const Value &pokemon,
                                          const std::string &slot, std::size_t attack_index,
                                          const Value &cards) {
    Value simulation = state;
    if (!place_candidate_in_active(simulation, actor, pokemon, slot))
        return 0;
    return estimated_attack_damage(position, simulation, actor, attack_index, cards);
}

std::int64_t pokemon_attack_damage_ceiling(const RulesSession &position, const Value &state,
                                           std::int32_t actor, const Value &pokemon,
                                           const std::string &slot, std::size_t attack_index,
                                           const Value &cards) {
    const Value *definition = card(cards, string_field(pokemon, "card_id"));
    ...
    for (const Value *effect : flatten_effects(array_field(attack, "effects"))) {
        ...
        const Value *params = field(*effect, "params");
        if (params != nullptr && params->is_object()) {
            potential += integer_field(*params, "bonus", integer_field(*params, "amount"));
        }
    }
    return std::max(potential, estimated_damage_for_pokemon(position, state, actor, pokemon, slot,
                                                            attack_index, cards));
}

std::int64_t best_pokemon_damage_for_state(const RulesSession &position, const Value &state,
                                           std::int32_t actor, const Value &pokemon,
                                           const std::string &slot, const Value &cards) {
    const Value *definition = card(cards, string_field(pokemon, "card_id"));
    const auto &attacks =
    ...
    std::int64_t best = 0;
    for (std::size_t index = 0; index < attacks.size(); ++index) {
        best = std::max(best, pokemon_attack_damage_ceiling(position, state, actor, pokemon, slot,
                                                            index, cards));
    }
    return best;
}

HighImpactPlan high_impact_attack_plan(const RulesSession &position, const Value &state,
                                       std::int32_t actor, const Value &pokemon,
                                       const std::string &slot, const std::string &energy_card_id,
                                       const Value &cards) {
    const Value probe = !energy_card_id.empty() && is_energy(cards, energy_card_id)
                            ? pokemon_with_extra_energy(pokemon, energy_card_id)
                            : pokemon;
    ...
    for (std::size_t index = 0; index < attacks.size(); ++index) {
        const std::int64_t missing =
            missing_energy(cards, probe, array_field(attacks[index], "cost"));
        const std::int64_t damage =
            pokemon_attack_damage_ceiling(position, state, actor, probe, slot, index, cards);
        const double impact = static_cast<double>(damage) - static_cast<double>(missing) * 50.0;
        if (impact > best.impact ||
            (impact == best.impact &&
             (missing < best.missing || (missing == best.missing && damage > best.damage)))) {
            best.damage = damage;
...
std::int64_t high_impact_missing_energy(const RulesSession &position, const Value &state,
                                        std::int32_t actor, const Value &pokemon,
                                        const std::string &slot, const std::string &energy_card_id,
                                        const Value &cards) {
    return high_impact_attack_plan(position, state, actor, pokemon, slot, energy_card_id, cards)
        .missing;
}
...
bool commands_require_belief_sampling(const Value &commands) {
    ...
    static const std::set<std::string> belief_ops{
        "draw_and_attach_energy", "look_top_attach_energy", "look_top_deck",
        "mill_then_damage",       "trekking_shoes",
    };
    for (const Value &command : commands.as_array()) {
        if (!command.is_object())
            continue;
        const std::string op = string_field(command, "op");
        if (op.rfind("flip_coin", 0) == 0 || op == "flip_until_tails" || belief_ops.count(op))
            return true;
        ...
        for (const auto &[branch_name, branch] : branches->as_object()) {
            (void)branch_name;
            if (commands_require_belief_sampling(branch))
                return true;
        }
    }
    return false;
...
std::int64_t best_deterministic_available_damage(const RulesSession &position, const Value &state,
                                                 std::int32_t actor, const Value &cards) {
    const Value *pokemon = active(state, actor);
    ...
    for (std::size_t index = 0; index < attacks.size(); ++index) {
        if (missing_energy(cards, *pokemon, array_field(attacks[index], "cost")) <= 0 &&
            !commands_require_belief_sampling(field(attacks[index], "compiled_effects") == nullptr
                                                  ? Value::make_array()
                                                  : *field(attacks[index], "compiled_effects"))) {
            best = std::max(best, estimated_attack_damage(position, state, actor, index, cards));
        }
    }
    return best;
...
double energy_plan_target_bonus(const RulesSession &position, const Value &state,
                                std::int32_t actor, const Value &pokemon, const std::string &slot,
                                const std::string &energy_card_id, const std::string &key,
                                const Value &cards) {
    const Value *definition = card(cards, string_field(pokemon, "card_id"));
    if (definition == nullptr)
    ...
    double bonus = 0.0;
    if (deck.contains("core", pokemon_id))
        bonus += 95.0;
    if (deck.contains("evolution", pokemon_id) ||
        !array_field(pokemon, "evolution_stack_ids").empty())
        bonus += 45.0;

    const std::int64_t damage_ceiling =
        best_pokemon_damage_for_state(position, state, actor, pokemon, slot, cards);
    if (deck.contains("engine", pokemon_id) && !deck.contains("core", pokemon_id)) {
        ...
        if (damage_ceiling < deck.high_impact_damage_floor())
            bonus -= 25.0;
    }
    if (slot != "active" && deck.contains("bench", pokemon_id))
        bonus += 34.0;
    if (has_subtype(*definition, "ex"))
        bonus += 45.0;
    bonus += std::min(120.0, static_cast<double>(damage_ceiling) * 0.35);

    const std::int64_t missing = best_missing(cards, &pokemon);
    const std::int64_t high_before =
        high_impact_missing_energy(position, state, actor, pokemon, slot, {}, cards);
    const std::int64_t high_after =
        !energy_card_id.empty() && is_energy(cards, energy_card_id)
            ? high_impact_missing_energy(position, state, actor, pokemon, slot, energy_card_id,
                                         cards)
            : high_before;
    const std::int64_t high_progress = std::max<std::int64_t>(0, high_before - high_after);
    if (damage_ceiling >= deck.high_impact_damage_floor() && high_progress > 0) {
        ...
        if (slot != "active" && deck.contains("core", pokemon_id))
            bonus += 75.0;
    }
    if (missing == 0)
        bonus += 35.0;
    ...
    if (slot == "active" &&
        ...
        missing > 0)
        bonus -= 65.0;
    if (!energy_card_id.empty() && energy_matches_profile(cards, energy_card_id, key))
        bonus += 18.0;

    return bonus;
}

bool has_better_bench_energy_plan(const RulesSession &position, const Value &state,
                                  std::int32_t actor, const std::string &energy_card_id,
                                  const std::string &key, const Value &cards) {
    const Value &owner = player(state, actor);
    const Value *active_pokemon = active(state, actor);
    if (active_pokemon == nullptr)
        return false;
    const DeckProfile deck = profile(cards, key);
    const Value *active_definition = card(cards, string_field(*active_pokemon, "card_id"));
    const std::int64_t active_max_hp =
        active_definition == nullptr ? 0 : integer_field(*active_definition, "hp");
    const std::int64_t active_damage =
        best_pokemon_damage_for_state(position, state, actor, *active_pokemon, "active", cards);
    if (deck.contains("core", string_field(*active_pokemon, "card_id")) &&
    ...
    const std::int64_t active_after =
        best_missing_with_extra(cards, active_pokemon, energy_card_id);
    const std::int64_t active_power_before =
        high_impact_missing_energy(position, state, actor, *active_pokemon, "active", {}, cards);
    const std::int64_t active_power_after = high_impact_missing_energy(
        position, state, actor, *active_pokemon, "active", energy_card_id, cards);
    const bool active_progresses =
        active_after < active_before || (active_damage >= deck.high_impact_damage_floor() &&
    ...
    for (std::size_t index = 0; index < bench.size(); ++index) {
        ...
        const std::int64_t after = best_missing_with_extra(cards, &pokemon, energy_card_id);
        const std::int64_t damage =
            best_pokemon_damage_for_state(position, state, actor, pokemon, slot, cards);
        const std::int64_t power_before =
            high_impact_missing_energy(position, state, actor, pokemon, slot, {}, cards);
        const std::int64_t power_after = high_impact_missing_energy(position, state, actor, pokemon,
                                                                    slot, energy_card_id, cards);
        const bool progresses = after < before || (damage >= deck.high_impact_damage_floor() &&
                                                   power_after < power_before);
        if (progresses && deck.contains("core", string_field(pokemon, "card_id")) &&
            (!active_progresses || active_damage < damage))
            return true;
        if (progresses && before > 0 && after == 0 &&
            damage >= std::max<std::int64_t>(deck.high_impact_damage_floor(), active_damage + 40))
            return true;
    }
    return false;
}

...
bool ChallengeSearchProviderImpl::duplicate_energy_choice_response(
    ...
    if (options_value == nullptr || !options_value->is_array() || presentation_value == nullptr ||
        !presentation_value->is_object()) {
        return false;
    }
    ...
    if (same_target) {
        ...
        for (std::size_t index = 0; index < options.size(); ++index) {
            ...
            const ptcg::ai::Value plan = card_evaluator_.energy_target_prefix_plan(
                position, choice.player, pending, options[index], maximum);
            ...
            double score =
                count <= 0
                    ? -10000.0
                    : base_score +
                          std::max(0.0, std::min(180.0, (plan.find("gain") == nullptr
                                                             ? 0.0
                                                             : plan.find("gain")->as_number()) *
                                                            0.35));
            score += strategy_catalog_.choice_score(position.search_state(), choice.player, pending,
                                                    options[index]);
            ranked.push_back({index, score, count});
        }
        const auto approximately_equal = [](double left, double right) {
            const double tolerance =
                0.00001 * std::max(1.0, std::max(std::abs(left), std::abs(right)));
        ...
        if (selected_row != nullptr) {
            ...
            if (selected_row->useful_count >= 0) {
                count = std::max(minimum, std::min(maximum, selected_row->useful_count));
            }
            count = std::min(count, max_per_target);
            const std::string option_id = string_field(options[selected_row->index], "option_id");
            for (std::int64_t index = 0; index < count; ++index) {
                selected.emplace_back(option_id);
            }
        }
        if (selected.size() < static_cast<std::size_t>(minimum))
            return false;
    ...
    const auto generic_public_distribution = [&]() {
        ...
        for (std::size_t index = 0; index < options.size(); ++index) {
            ...
            ranked.push_back({
                index,
                *base + strategy_catalog_.choice_score(position.search_state(), choice.player,
                                                       pending, options[index]),
            });
        }
        std::stable_sort(
            ranked.begin(), ranked.end(), [](const RankedOption &left, const RankedOption &right) {
                const double tolerance =
                    0.00001 * std::max(1.0, std::max(std::abs(left.score), std::abs(right.score)));
        ...
        const std::int64_t target_count =
            minimum <= 0 ? (allowed.empty() ? 0 : maximum) : std::max(minimum, maximum);
        ptcg::ai::Value::Array selected;
        ...
        while (selected.size() < static_cast<std::size_t>(target_count)) {
            ...
            for (const std::size_t option_index : allowed) {
                const std::string option_id = string_field(options[option_index], "option_id");
                const ptcg::ai::Value *reference = options[option_index].find("ref");
                const std::string target_key =
                    reference != nullptr && reference->is_object()
                        ? std::to_string(integer_field(*reference, "player", choice.player)) + ":" +
                              string_field(*reference, "slot")
                        : option_id;
                if (option_id.empty() ||
                    (same_target && !selected_target.empty() && target_key != selected_target) ||
                    per_target[target_key] >= max_per_target)
                    continue;
                selected.emplace_back(option_id);
    ...
    if (card_ids_value == nullptr || !card_ids_value->is_array() ||
        card_ids_value->as_array().size() < static_cast<std::size_t>(maximum)) {
    ...
    for (std::int64_t index = 0; index < maximum; ++index) {
        const std::string id =
            card_ids_value->as_array()[static_cast<std::size_t>(index)].string_or();
        const ptcg::ai::Value *definition = std::as_const(cards_).find(id);
        if (definition == nullptr || string_field(*definition, "supertype") != "Energy")
            return false;
    ...
    if (players == nullptr || !players->is_array() || choice.player < 0 ||
        ...
        return false;
    const ptcg::ai::Value &owner = players->as_array()[static_cast<std::size_t>(choice.player)];
    for (std::size_t index = 0; index < options.size(); ++index) {
        ...
        const std::string slot = reference != nullptr && reference->is_object()
                                     ? string_field(*reference, "slot")
                                     : std::string{};
        const std::int64_t target_player = reference != nullptr && reference->is_object()
                                               ? integer_field(*reference, "player", choice.player)
                                               : choice.player;
        if (option_id.empty() || slot.empty() || target_player != choice.player ||
            seen_slots.count(slot) || seen_ids.count(option_id))
            continue;
        const ptcg::ai::Value *pokemon = nullptr;
        if (slot == "active")
            pokemon = owner.find("active");
        else if (slot.rfind("bench_", 0) == 0) {
            try {
                const std::size_t bench_index =
                    static_cast<std::size_t>(std::stoll(slot.substr(6)));
        ...
        if (pokemon == nullptr || !pokemon->is_object())
            continue;
        const std::string public_card = reference != nullptr && reference->is_object()
                                            ? string_field(*reference, "card_id")
                                            : std::string{};
        if (!public_card.empty() && public_card != string_field(*pokemon, "card_id"))
            continue;
    ...
    const bool relocation =
        purpose.rfind("energy_relocate", 0) == 0 || purpose.rfind("relocate_energy", 0) == 0;
    if (relocation && minimum != maximum)
        return false;
    ...
    for (std::int64_t count = minimum; count <= maximum; ++count) {
        ...
        for (std::uint64_t ordinal = 0; ordinal < assignment_count; ++ordinal) {
            ...
            // Decode in base N with the last position changing fastest.
            for (std::size_t reverse = assignment.size(); reverse > 0; --reverse) {
                assignment[reverse - 1] = static_cast<std::size_t>(encoded % targets.size());
            ...
            ptcg::ai::Value &mutable_owner =
                snapshot_players->as_array()[static_cast<std::size_t>(choice.player)];
            if (relocation) {
                const std::int32_t source_player = static_cast<std::int32_t>(
                    integer_field(presentation, "source_player", choice.player));
                const std::string source_slot = string_field(presentation, "source_slot");
                const ptcg::ai::Value *refs = presentation.find("attachment_refs");
                if (source_player != choice.player || source_slot.empty() || refs == nullptr ||
                    !refs->is_array() ||
                    refs->as_array().size() < static_cast<std::size_t>(maximum)) {
                    return false;
                }
                ptcg::ai::Value *source =
                    source_slot == "active" ? mutable_owner.find("active") : nullptr;
                if (source == nullptr && source_slot.rfind("bench_", 0) == 0) {
                    try {
                        const std::size_t index =
                            static_cast<std::size_t>(std::stoll(source_slot.substr(6)));
                ...
                for (std::int64_t index = 0; index < maximum; ++index) {
                    const ptcg::ai::Value &reference =
                        refs->as_array()[static_cast<std::size_t>(index)];
                    const std::int64_t attachment_index = integer_field(reference, "index", -1);
                    if (!reference.is_object() || attachment_index < 0 ||
                        static_cast<std::size_t>(attachment_index) >= attached->as_array().size() ||
                        ...
                        attached->as_array()[static_cast<std::size_t>(attachment_index)]
                                .string_or() != energy_ids[static_cast<std::size_t>(index)])
                        return false;
            ...
            for (std::size_t position_index = 0; position_index < assignment.size();
                 ++position_index) {
                const Target &target = targets[assignment[position_index]];
                candidate_ids.emplace_back(target.option_id);
                ptcg::ai::Value *pokemon =
                    target.slot == "active" ? mutable_owner.find("active") : nullptr;
                if (pokemon == nullptr && target.slot.rfind("bench_", 0) == 0) {
                    const std::size_t index =
                        static_cast<std::size_t>(std::stoll(target.slot.substr(6)));
                    ptcg::ai::Value *bench = mutable_owner.find("bench");
                    if (bench != nullptr && bench->is_array() && index < bench->as_array().size()) {
                ...
                if (pokemon != nullptr && pokemon->is_object()) {
                    ...
                    if (attached == nullptr || !attached->is_array()) {
                        (*pokemon)["energy_card_ids"] = ptcg::ai::Value::make_array();
                        attached = pokemon->find("energy_card_ids");
                    }
                    attached->as_array().emplace_back(energy_ids[position_index]);
                }
            }
            std::string restore_error;
            if (!simulation->restore(snapshot, position.rng_state(), &restore_error))
                return false;
            const double score =
                card_evaluator_.energy_distribution_board_utility(*simulation, choice.player);
...
bool ChallengeSearchProviderImpl::retreat_payment_response(const ptcg::ai::Value &state,
                                                           const ptcg::ai::Value &pending,
                                                           ptcg::ai::Value &response) const {
    const ptcg::ai::Value *presentation = pending.find("presentation");
    const std::int64_t required_units =
        std::max<std::int64_t>(0, presentation != nullptr && presentation->is_object()
                                      ? integer_field(*presentation, "required_units")
                                      : 0);
    ptcg::ai::Value::Array selected_ids;
    bool cancelled = false;
    const auto finish = [&]() {
        response = ptcg::ai::Value(ptcg::ai::Value::Object{
            {"request_id", ptcg::ai::Value(string_field(pending, "request_id"))},
    ...
    if (required_units <= 0)
        return finish();
    const std::int32_t actor = static_cast<std::int32_t>(integer_field(pending, "player", -1));
    const ptcg::ai::Value *players = state.find("players");
    if (actor < 0 || actor > 1 || players == nullptr || !players->is_array() ||
    ...
    const ptcg::ai::Value *energy_value =
        active != nullptr && active->is_object() ? active->find("energy_card_ids") : nullptr;
    ...
    for (std::size_t order = 0; order < options.size(); ++order) {
        ...
        if (string_field(*ref, "kind") != "attachment" ||
            string_field(*ref, "attachment_type") != "energy" ||
            integer_field(*ref, "player", -1) != actor || string_field(*ref, "slot") != "active" ||
            index < 0 || static_cast<std::size_t>(index) >= energy_ids.size() ||
            string_field(*ref, "card_id") !=
                energy_ids[static_cast<std::size_t>(index)].string_or())
            continue;
        const std::int64_t units =
            energy_units_provided_by_card(energy_ids, static_cast<std::size_t>(index));
        if (units <= 0)
            continue;
...
std::int64_t
ChallengeSearchProviderImpl::energy_units_provided_by_card(const ptcg::ai::Value::Array &attached,
                                                           std::size_t index) const {
    if (index >= attached.size())
        return 0;
    const ptcg::ai::Value *definition = std::as_const(cards_).find(attached[index].string_or());

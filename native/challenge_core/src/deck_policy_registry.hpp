...
class DeckPolicyRegistry {
  public:
    double resource_bundle_value(const Value &state, std::int32_t actor, const Value &retained_hand,
                                 double card_utility, double resource_delta) const;
    DeckPolicyRegistry(Value strategies, Value catalog);
    ...
    double action_score(const Value &state, std::int32_t actor, const Value &action) const;
    double choice_score(const Value &state, std::int32_t actor, const Value &choice_view,
    ...
    bool card_has_role(const Value &state, std::int32_t actor, const std::string &card_id,
                       const std::string &role) const;

    const DeckPolicy &policy_for(const std::string &deck_key) const;
    double position_value(const Value &state, std::int32_t actor,
                          const PositionFeatures &facts) const;

  private:
    std::map<std::string, std::shared_ptr<const DeckPolicy>> policies_;
    std::shared_ptr<const DeckPolicy> generic_;

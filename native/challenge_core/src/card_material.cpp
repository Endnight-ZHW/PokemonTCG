...
std::int64_t missing_energy_units(const std::vector<std::string> &available, const Value *cost) {
    return cost == nullptr || !cost->is_array()
               ? 0
               : traditional_card::missing_energy_units(available, cost->as_array());
}

bool modifier_operation_matches(const Value &pokemon, const std::string &kind,
    ...
    for (const Value &modifier : modifiers->as_array()) {
        ...
        if (operation == nullptr || !operation->is_object() ||
            string_field(*operation, "kind") != kind)
            continue;
        if (text.empty())
            return true;
        const std::string operation_text =
            string_field(*operation, "attack_name", string_field(*operation, "reason"));
        if (operation_text == text || operation_text == "__all__")
            return true;
    }
    return false;
}
...
AttackProfile public_attack_profile(const Value &cards, const Value &pokemon) {
    ...
    const Value *attacks = definition == nullptr ? nullptr : field(*definition, "attacks");
    ...
    if (attacks != nullptr && attacks->is_array()) {
        for (const Value &attack : attacks->as_array()) {
            ...
            const std::int64_t cost_size = cost != nullptr && cost->is_array()
                                               ? static_cast<std::int64_t>(cost->as_array().size())
                                               : 0;
            const std::int64_t missing = missing_energy_units(available, cost);
            result.useful_units =
                std::max(result.useful_units, std::max<std::int64_t>(0, cost_size - missing));
            const std::string name = string_field(attack, "name");
            if (modifier_operation_matches(pokemon, "attack_lock", name))
                continue;
            ++eligible;
    ...
    result.stranded_units = std::max<std::int64_t>(0, static_cast<std::int64_t>(available.size()) -
                                                          result.useful_units);
    result.ready_ratio =
        eligible > 0 ? static_cast<double>(ready) / static_cast<double>(eligible) : 0.0;
    ...
    if (array_contains(statuses, "ASLEEP") || array_contains(statuses, "PARALYZED")) {
        ...
        if (array_contains(statuses, "CONFUSED"))
...
double pokemon_board_score(const RulesSession &position, const Value &cards, const Value &pokemon,
                           bool active) {
    ...
    score += static_cast<double>(evolutions != nullptr && evolutions->is_array()
                                     ? evolutions->as_array().size()
                                     : 0U) *
             18.0;
    double preparation = 0.0;
    if (profile.minimum_missing == 0)
        preparation = 1.0;
    else if (profile.minimum_missing == 1)
        preparation = 0.55;
    else if (profile.minimum_missing == 2)
        preparation = 0.20;
...
double board_score(const RulesSession &position, const Value &cards, const Value &player) {
    double score = 0.0;
...
double expected_attack_damage(const Value &attack) {
    ...
    for (const Value &command : commands->as_array()) {
        ...
        if (op == "flip_coin_repeat_damage") {
            expected = static_cast<double>(integer_field(*args, "flips")) * 0.5 *
...
std::int64_t CardEvaluator::base_state_score_milli(const RulesSession &position,
    ...
    if (string_field(state, "result_status", "ONGOING") != "ONGOING" ||
        ...
        if (string_field(state, "result_status") == "DRAW")
            return 0;
        return integer_field(state, "winner", -1) == actor ? 1000000000LL : -1000000000LL;
    }
    ...
    double score = static_cast<double>(static_cast<std::int64_t>(array_size(opponent, "prizes")) -
                                       static_cast<std::int64_t>(array_size(own, "prizes"))) *
                   150.0;
    score += static_cast<double>(static_cast<std::int64_t>(array_size(own, "hand")) -
                                 static_cast<std::int64_t>(array_size(opponent, "hand"))) *
             2.0;
    score += static_cast<double>(static_cast<std::int64_t>(array_size(own, "deck")) -
                                 static_cast<std::int64_t>(array_size(opponent, "deck"))) *
             0.05;
    score += board_score(position, *cards_.find("cards"), own) -
             board_score(position, *cards_.find("cards"), opponent);
    if (bool_field(own, "retreated_this_turn"))
        score -= 42.0;
    if (bool_field(opponent, "retreated_this_turn"))
        score += 42.0;
    return quantize(score);
}

std::int64_t CardEvaluator::default_action_score_milli(const Value &action, const Value &catalog) {
    ...
    if (kind == "DECLARE_ATTACK") {
        ...
        const Value *definition =
            source == nullptr ? nullptr : card(catalog, string_field(*source, "card_id"));
        const Value *attacks = definition == nullptr ? nullptr : field(*definition, "attacks");
        const std::int64_t attack_index =
            payload == nullptr ? -1 : integer_field(*payload, "attack_index", -1);
        if (attacks != nullptr && attacks->is_array() && attack_index >= 0 &&
            static_cast<std::size_t>(attack_index) < attacks->as_array().size()) {
            priority +=
                expected_attack_damage(attacks->as_array()[static_cast<std::size_t>(attack_index)]);
        }
        ...
        priority = 180.0;
    else if (kind == "RETREAT" || kind == "PROMOTE")
        priority = 140.0;
...
std::int64_t CardEvaluator::quantize(double score) noexcept {

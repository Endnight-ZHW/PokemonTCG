...
// deck_planner_v1 path.  It accepts only a public observation (either the
...
class InformationSet {
  public:
    ...
    bool capture(const Value &public_state, std::int32_t perspective, const Value &catalog,
                 const Value &decks, const Value &legal_actions, const Value &public_history,
                 std::int64_t match_seed, std::string *error = nullptr);
    ...
    std::size_t recommended_belief_samples(std::int32_t player, std::size_t requested) const;
    ...
    bool apply_deck_inspection(const Value &choice_view, std::string *error = nullptr);
    bool apply_known_prizes(const Value::Array &prize_cards, std::string *error = nullptr);

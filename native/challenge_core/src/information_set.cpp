...
bool is_hidden_id(std::string_view card_id) noexcept {
    return card_id == "__ai_hidden_card__" || card_id == "__ai_hidden_prize__" ||
           card_id == "__hidden_card__" || card_id == "__hidden_prize__";
}
...
Array history_card_ids(const Value &event, const Value &data) {
    ...
    if (entries != nullptr && entries->is_array()) {
        ...
        for (const Value &entry : entries->as_array()) {
            const std::string card_id =
                entry.is_object() ? string_field(entry, "card_id") : entry.string_or();
    ...
    if (result.empty()) {
        const std::string card_id = string_field(event, "card_id", string_field(data, "card_id"));
        if (!card_id.empty() && !is_hidden_id(card_id)) {
            result.emplace_back(card_id);
        }
    }
...
std::int32_t history_endpoint_player(const Value &event, const Value &data,
...
std::string history_endpoint_zone(const Value &event, const Value &data, const char *endpoint_key,
                                  const char *data_key) {
    const Value *endpoint = field(event, endpoint_key);
    if (endpoint != nullptr && endpoint->is_object()) {
...
bool is_hand_movement_event(const std::string &event_type) {
    return event_type == "card_moved" || event_type == "cards_discarded" ||
           event_type == "cards_selected" || event_type == "energy_attached" ||
           event_type == "pokemon_evolved" || event_type == "pokemon_played" ||
           event_type == "stadium_changed" || event_type == "tool_attached" ||
...
void remove_known_cards(Array &known, const Array &departed) {
    for (const Value &card : departed) {
        ...
        const auto found = std::find_if(known.begin(), known.end(), [&card_id](const Value &entry) {
...
bool replay_known_opponent_hand(const Value &public_history, std::int32_t perspective, Array &known,
                                std::string *error) {
    known.clear();
    if (!public_history.is_array() || public_history.as_array().size() > max_public_history) {
        if (error != nullptr)
            *error = "invalid_public_history";
        return false;
    }
    ...
    for (const Value &event : public_history.as_array()) {
        ...
        if (data_value == nullptr || !data_value->is_object() || source_value == nullptr ||
            !source_value->is_object() || target_value == nullptr || !target_value->is_object()) {
            if (error != nullptr)
                *error = "invalid_public_history";
            return false;
        }
        ...
        if (event_type.empty() ||
            (visibility != "public" && visibility != "owner" && visibility != "private")) {
            if (error != nullptr)
                *error = "invalid_public_history";
            return false;
        }
        ...
        const std::int32_t visibility_owner = static_cast<std::int32_t>(
            integer_field(data, "visibility_owner",
                          integer_field(data, "player", integer_field(event, "actor", -1))));
        const Array card_ids = history_card_ids(event, data);
        if ((visibility != "public" && visibility_owner != 0 && visibility_owner != 1)) {
            if (error != nullptr)
                *error = "invalid_public_history";
            return false;
        }
        if ((visibility == "private" && visibility_owner != perspective) ||
            (visibility != "public" && visibility_owner != perspective && !card_ids.empty())) {
            if (error != nullptr)
                *error = "private_public_history";
            return false;
        }
        const std::int32_t source_player = history_endpoint_player(event, data, "source");
        const std::int32_t target_player = history_endpoint_player(event, data, "target");
        const std::string source_zone = history_endpoint_zone(event, data, "source", "source_zone");
        const std::string target_zone = history_endpoint_zone(event, data, "target", "target_zone");

        if (source_player == opponent && source_zone == "hand" &&
        ...
        if (target_player == opponent && target_zone == "hand" && visibility == "public") {
...
Value normalize_pokemon(const Value *source) {
    ...
    Value result(Object{
        ...
        {"energy_card_ids", array_field_or(*source, "energy_card_ids", Value::make_array())},
        {"attached_tool_id", Value(string_field(*source, "attached_tool_id"))},
        {"status_conditions", array_field_or(*source, "status_conditions", Value::make_array())},
        {"evolution_stack_ids",
         array_field_or(*source, "evolution_stack_ids", Value::make_array())},
        {"can_evolve_this_turn", Value(bool_field(*source, "can_evolve_this_turn", true))},
        {"placed_this_turn", Value(bool_field(*source, "placed_this_turn", true))},
        {"used_abilities", array_field_or(*source, "used_abilities", Value::make_array())},
        {"healed_this_turn", Value(bool_field(*source, "healed_this_turn"))},
        {"paralyzed_since_turn", Value(integer_field(*source, "paralyzed_since_turn"))},
    });
    const Value *modifiers = field(*source, "modifiers");
    if (modifiers != nullptr && modifiers->is_array() && !modifiers->as_array().empty()) {
...
Value normalize_player(const Value &source) {
    ...
    for (std::size_t index = 0; index < 5; ++index) {
        const Value *pokemon = index < bench.as_array().size() ? &bench.as_array()[index] : nullptr;
    ...
    Value result(Object{
        ...
        {"supporter_played_this_turn", Value(bool_field(source, "supporter_played_this_turn"))},
        {"energy_attached_this_turn", Value(bool_field(source, "energy_attached_this_turn"))},
        {"retreated_this_turn", Value(bool_field(source, "retreated_this_turn"))},
        {"stadium_played_this_turn", Value(bool_field(source, "stadium_played_this_turn"))},
        {"stadium_used_this_turn", Value(bool_field(source, "stadium_used_this_turn"))},
        {"healed_this_turn", Value(bool_field(source, "healed_this_turn"))},
...
Value normalize_public_snapshot(const Value &source, std::int32_t perspective,
                                const Value &legal_actions, std::int64_t match_seed) {
    ...
    const std::string setup_stage = string_field(source, "setup_stage", "INITIAL_PLACEMENT");
    Value rules_options = object_field_or(source, "rules_options", Value::make_object());
    const bool type_matchups =
        bool_field(rules_options, "apply_type_matchups", bool_field(source, "apply_type_matchups"));
    ...
    Value result(Object{
        ...
        {"stadium_owner_idx", Value(integer_field(source, "stadium_owner_idx", -1))},
        {"winner", Value(winner)},
        {"result_status",
         Value(string_field(source, "result_status", winner >= 0 ? "WIN" : "ONGOING"))},
        {"result_reason", Value(string_field(source, "result_reason"))},
        {"result_conditions",
         pair_array(array_field_or(source, "result_conditions", Value::make_array()),
        ...
        {"public_deck_keys",
         pair_array(array_field_or(source, "public_deck_keys", Value::make_array()), Value(""),
                    Value(""))},
        {"apply_type_matchups", Value(type_matchups)},
        {"rules_profile_id", Value(string_field(source, "rules_profile_id", "CN_MAINLAND_3_1_0"))},
        {"rules_options", std::move(rules_options)},
        {"action_log", Value::make_array()},
        {"mulligan_count", pair_array(array_field_or(source, "mulligan_count", Value::make_array()),
                                      Value(0), Value(0))},
        {"extra_draws", pair_array(array_field_or(source, "extra_draws", Value::make_array()),
                                   Value(0), Value(0))},
        {"setup_ready", pair_array(array_field_or(source, "setup_ready", Value::make_array()),
                                   Value(false), Value(false))},
        {"setup_stage", Value(setup_stage)},
        {"setup_actor_idx", Value(integer_field(source, "setup_actor_idx", first_player))},
        {"opening_coin_winner_idx",
         Value(integer_field(source, "opening_coin_winner_idx", first_player))},
        {"mulligan_bonus_max", Value(integer_field(source, "mulligan_bonus_max"))},
        {"setup_bonus_card_ids",
         pair_array(array_field_or(source, "setup_bonus_card_ids", Value::make_array()),
                    Value::make_array(), Value::make_array())},
        {"pending_promotions", array_field_or(source, "pending_promotions", Value::make_array())},
        {"processed_action_ids", Value::make_array()},
        {"resolution_stack", empty_resolution_stack()},
        {"turn_fact_book", object_field_or(source, "turn_fact_book", empty_turn_fact_book())},
        {"snapshot_version", Value(3)},
        {"perspective", Value(perspective)},
        {"actor", Value(perspective)},
        {"match_seed", Value(match_seed)},
        {"legal_actions", legal_actions.is_array() ? legal_actions : Value::make_array()},
    });
    ...
    for (std::int32_t player = 0; player < 2; ++player) {
        ...
        if (player != perspective) {
            row["hand"] = hidden_cards(row["hand"].as_array().size(), "__ai_hidden_card__");
        }
    }
    ...
    if (promotions != nullptr && promotions->is_array() && !promotions->as_array().empty()) {
        ...
        if (pending_actor == 0 || pending_actor == 1)
            result["actor"] = Value(pending_actor);
    } else if (string_field(result, "phase") == "SETUP") {
        const std::int64_t setup_actor = integer_field(result, "setup_actor_idx", -1);
        if (setup_actor == 0 || setup_actor == 1)
            result["actor"] = Value(setup_actor);
    } else {
...
bool InformationSet::capture(const Value &public_state, std::int32_t perspective,
                             const Value &catalog, const Value &decks, const Value &legal_actions,
                             const Value &public_history, std::int64_t match_seed,
                             std::string *error) {
    valid_ = false;
    remaining_pools_ = {};
    known_hands_ = {};
    ...
    if (!public_state.is_object() || !decks.is_object()) {
        if (error != nullptr)
            *error = "invalid_public_state";
        return false;
    }
    if (!replay_known_opponent_hand(public_history, perspective,
    ...
    try {
        public_snapshot_ =
            normalize_public_snapshot(public_state, perspective, legal_actions, match_seed);
        ...
        if (cards.find("sv1-ener-1") != nullptr) {
            ...
            for (const auto &[card_id, card] : cards.as_object()) {
                if (string_field(card, "supertype") != "Energy")
                    continue;
                const Value *subtypes = field(card, "subtypes");
                if (subtypes == nullptr || !subtypes->is_array())
                    continue;
        ...
        for (std::int32_t player = 0; player < 2; ++player) {
            ...
            Array pool =
                expand_deck(decks, deck_keys[static_cast<std::size_t>(player)].string_or());
            ...
            if (!known_hand.empty()) {
                ...
                for (const Value &known_card : known_hand) {
                    ...
                    const auto found =
                        std::find_if(pool.begin(), pool.end(), [&card_id](const Value &entry) {
                            return entry.string_or() == card_id;
                        });
                    if (found == pool.end()) {
                        if (error != nullptr)
            ...
            const std::size_t hidden_count = hand_count - known_hand.size() +
...
bool InformationSet::valid() const noexcept {
    return valid_;
}
std::int32_t InformationSet::perspective() const noexcept {
    return perspective_;
}
std::int64_t InformationSet::match_seed() const noexcept {
    return match_seed_;
}
const Value &InformationSet::public_snapshot() const noexcept {
    return public_snapshot_;
}

const Value::Array &InformationSet::remaining_pool(std::int32_t player) const {
    if (player != 0 && player != 1)
        throw std::out_of_range("invalid_player");
    return remaining_pools_[static_cast<std::size_t>(player)];
}

const Value::Array &InformationSet::known_hand(std::int32_t player) const {
    if (player != 0 && player != 1)
        throw std::out_of_range("invalid_player");
    return known_hands_[static_cast<std::size_t>(player)];
}

std::size_t InformationSet::hand_count(std::int32_t player) const {
    if (player != 0 && player != 1)
        throw std::out_of_range("invalid_player");
    ...
    const Value *hand = players->as_array()[static_cast<std::size_t>(player)].find("hand");
    return hand != nullptr && hand->is_array() ? hand->as_array().size() : 0;
}

std::size_t InformationSet::unknown_hand_count(std::int32_t player) const {
    const std::size_t total = hand_count(player);
    const std::size_t known = known_hand(player).size();
    return total > known ? total - known : 0;
}

std::size_t InformationSet::recommended_belief_samples(std::int32_t player,
    ...
    if (players != nullptr && players->is_array() &&
        static_cast<std::size_t>(player) < players->as_array().size()) {
        const Value *deck = players->as_array()[static_cast<std::size_t>(player)].find("deck");
    ...
    if (unknown == 0 && deck_count == 0)
        return 1;
    return std::min<std::size_t>(requested, 2);
}

bool InformationSet::has_published_deck(std::int32_t player) const noexcept {
    return (player == 0 || player == 1) && published_deck_valid_[static_cast<std::size_t>(player)];
}

bool InformationSet::apply_known_prizes(const Value::Array &prize_cards, std::string *error) {
    if (!valid_ || perspective_ < 0 || perspective_ > 1) {
        if (error != nullptr)
            *error = "invalid_information_set";
        return false;
    }
    const std::size_t player = static_cast<std::size_t>(perspective_);
    const Value *players = public_snapshot_.find("players");
    if (players == nullptr || !players->is_array() || player >= players->as_array().size()) {
        if (error != nullptr)
            *error = "invalid_player_snapshot";
        return false;
    }
    const Value &row = players->as_array()[player];
    const Value *deck = row.find("deck");
    const Value *prizes = row.find("prizes");
    if (!published_deck_valid_[player] || deck == nullptr || !deck->is_array() ||
        prizes == nullptr || !prizes->is_array() ||
        prize_cards.size() != prizes->as_array().size() || prize_cards.size() > 60) {
        if (error != nullptr)
            *error = "deck_inspection_memory_mismatch";
        return false;
    }
    Array exact_deck = remaining_pools_[player];
    if (exact_deck.size() != deck->as_array().size() + prize_cards.size() ||
        !subtract_cards(exact_deck, prize_cards) || exact_deck.size() != deck->as_array().size()) {
        if (error != nullptr)
            *error = "deck_inspection_memory_mismatch";
        return false;
    }
...
bool InformationSet::apply_deck_inspection(const Value &choice_view, std::string *error) {
    if (!valid_ || perspective_ < 0 || perspective_ > 1) {
        if (error != nullptr)
            *error = "invalid_information_set";
        return false;
    ...
    const Value *references =
        presentation != nullptr ? field(*presentation, "browse_card_refs") : nullptr;
    if (!choice_view.is_object() || integer_field(choice_view, "player", -1) != perspective_ ||
        presentation == nullptr || !presentation->is_object() ||
        integer_field(*presentation, "source_player", -1) != perspective_ ||
        string_field(*presentation, "source_zone") != "deck" || references == nullptr ||
        !references->is_array() || references->as_array().size() > 60) {
        if (error != nullptr)
            *error = "invalid_deck_inspection";
        return false;
    }
    const Value *players = public_snapshot_.find("players");
    if (players == nullptr || !players->is_array() || player >= players->as_array().size()) {
    ...
    if (!published_deck_valid_[player] || deck == nullptr || !deck->is_array() ||
        ...
        if (error != nullptr)
            *error = "deck_inspection_count_mismatch";
        return false;
    }
    ...
    for (const Value &reference : references->as_array()) {
        if (!reference.is_object() || reference.as_object().size() != 5 ||
            ...
            if (error != nullptr)
                *error = "invalid_deck_inspection_ref";
            return false;
        }
        const std::int64_t index =
            integer_field(reference, "index", std::numeric_limits<std::int64_t>::min());
        const std::string card_id = string_field(reference, "card_id");
        if (index < 0 || static_cast<std::size_t>(index) >= exact_deck.size() ||
            seen[static_cast<std::size_t>(index)] || card_id.empty() || is_hidden_id(card_id)) {
            if (error != nullptr)
                *error = "invalid_deck_inspection_ref";
            return false;
        }
    ...
    if (exact_prizes.size() != exact_deck.size() + prizes->as_array().size() ||
        !subtract_cards(exact_prizes, exact_deck) ||
        exact_prizes.size() != prizes->as_array().size()) {
        if (error != nullptr)
            *error = "deck_inspection_cards_mismatch";
        return false;
    }
    exact_decks_[player] = std::move(exact_deck);
...
bool InformationSet::has_exact_hidden_zones(std::int32_t player) const noexcept {
    return (player == 0 || player == 1) &&
           exact_hidden_zones_valid_[static_cast<std::size_t>(player)];
}

const Value::Array &InformationSet::known_deck(std::int32_t player) const {
    if (player != 0 && player != 1)
        throw std::out_of_range("invalid_player");
    return exact_decks_[static_cast<std::size_t>(player)];
}

const Value::Array &InformationSet::known_prizes(std::int32_t player) const {
    if (player != 0 && player != 1)
        throw std::out_of_range("invalid_player");
    return exact_prizes_[static_cast<std::size_t>(player)];
}

Value InformationSet::sample_state(std::uint32_t seed) const {
    if (!valid_)
        return Value();
    ...
    for (std::int32_t player = 0; player < 2; ++player) {
        ...
        const std::size_t total_hand_count =
            player == perspective_ ? 0U : row["hand"].as_array().size();
        ...
        while (pool.size() < needed)
            pool.emplace_back(fallback_card_id_);
        shuffle(pool, rng);
        if (pool.size() > needed)
            pool.resize(needed);
        std::size_t cursor = 0;
        if (player != perspective_) {
            ...
            const Array unknown_hand = slice(pool, cursor, cursor + hand_count);
            sampled_hand.insert(sampled_hand.end(), unknown_hand.begin(), unknown_hand.end());
            if (!known_hand.empty()) {

...
std::string row_signature(const RankedAction &row) {
    return row.signature;
}

std::string row_bucket(const RankedAction &row) {
    return row.semantic_bucket;
}

std::string row_purpose(const RankedAction &row) {
    return row.purpose_bucket;
}

std::size_t protected_replacement_index(const std::vector<RankedAction> &rows) {
    if (rows.empty())
        return 0;
    std::map<std::string, std::size_t> purpose_counts;
    for (const auto &row : rows)
        ++purpose_counts[row_purpose(row)];
    for (std::size_t offset = 0; offset < rows.size(); ++offset) {
        const std::size_t index = rows.size() - 1 - offset;
        if (purpose_counts[row_purpose(rows[index])] > 1)
            return index;
    }
    for (std::size_t offset = 0; offset < rows.size(); ++offset) {
        const std::size_t index = rows.size() - 1 - offset;
        if (!traditional_action_is_terminal(rows[index].action))
            return index;
    }
    return rows.size() - 1;
}

const RankedAction *best_unrepresented_target_variant(const std::vector<RankedAction> &ranked,
                                                      const std::vector<RankedAction> &selected) {
    ...
    for (const auto &row : ranked) {
        ...
        if (target_sensitive.find(purpose) == target_sensitive.end() ||
            selected_signatures.find(row_signature(row)) != selected_signatures.end()) {
            continue;
        }
...
std::ptrdiff_t target_variant_replacement_index(const std::vector<RankedAction> &rows,
                                                const std::string &protected_purpose,
                                                const std::string &best_signature) {
    std::ptrdiff_t result = -1;
    std::int64_t lowest_score = 1000000000LL;
    std::string lowest_signature;
    for (std::size_t index = 0; index < rows.size(); ++index) {
        const auto &row = rows[index];
        if (traditional_action_is_terminal(row.action) || row_purpose(row) == protected_purpose ||
            row_signature(row) == best_signature) {
            continue;
        }
        if (result < 0 || row.score_milli < lowest_score ||
            (row.score_milli == lowest_score && row_signature(row) > lowest_signature)) {
...
std::string traditional_semantic_bucket(const Value &action,
    ...
    const std::int64_t attack_index =
        integer_field(payload, "attack_index", integer_field(payload, "attack_idx", -1));
    const std::int64_t target_index =
        integer_field(payload, "target_index", integer_field(payload, "bench_index", -1));
    Value stable(Value::Object{
...
bool traditional_action_is_terminal(const Value &action) noexcept {
    ...
    const std::string kind = kind_value == nullptr ? std::string{} : kind_value->string_or();
...
std::vector<RankedAction> traditional_diverse_top_actions(const std::vector<RankedAction> &ranked,
                                                          std::size_t count, bool preserve_best) {
    ...
    for (const auto &row : ranked) {
        ...
        if (purpose.empty() || used_purposes.count(purpose) || used_signatures.count(signature))
            continue;
    ...
    for (const auto &row : ranked) {
        ...
        const std::string signature = row_signature(row);
        if (bucket.empty() || used_buckets.count(bucket) || used_signatures.count(signature))
            continue;
        result.push_back(row);
    ...
    for (const auto &row : ranked) {
        if (result.size() >= limit)
            break;
        if (used_signatures.count(row_signature(row)))
            continue;
        result.push_back(row);
        used_signatures.insert(row_signature(row));
    }
    const RankedAction *end_row = nullptr;
    for (const auto &row : ranked) {
    ...
    const bool has_terminal =
        std::any_of(result.begin(), result.end(), [](const RankedAction &row) {
            return traditional_action_is_terminal(row.action);
        });
    if (!has_terminal) {
        const auto terminal =
            std::find_if(ranked.begin(), ranked.end(), [](const RankedAction &row) {
                return traditional_action_is_terminal(row.action);
            });
        if (terminal != ranked.end()) {
            if (result.size() >= limit)
                result.back() = *terminal;
            else
                result.push_back(*terminal);
        }
    }
    const RankedAction *target_variant = best_unrepresented_target_variant(ranked, result);
    if (target_variant != nullptr) {
        if (result.size() < limit) {
            result.push_back(*target_variant);
        } else {
            // The policy decides whether its preferred route keeps this slot.
            const std::ptrdiff_t index = target_variant_replacement_index(
                result, target_variant->purpose_bucket, preserve_best ? row_signature(ranked.front()) : std::string{});
            if (index >= 0)
                result[static_cast<std::size_t>(index)] = *target_variant;
        }
    }
    return result;
}

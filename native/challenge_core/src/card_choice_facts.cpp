...
std::string choice_option_card_id(const Value &option, const Value &cards) {
    ...
    if (separator != std::string::npos && separator + 1 < option_id.size()) {
        ...
        if (card(cards, candidate) != nullptr)
            return candidate;
    }
    return {};
}
...
std::string choice_option_slot(const Value &option) {
    ...
    const std::size_t second =
        first == std::string::npos ? std::string::npos : id.find(':', first + 1);
    if (second == std::string::npos)
        return {};
    const std::string prefix = id.substr(0, first);
    return prefix == "pokemon" || prefix == "attachment" ? id.substr(second + 1) : std::string{};
}

std::int32_t choice_option_player(const Value &option, std::int32_t fallback) {
    const Value *reference = field(option, "ref");
    if (reference != nullptr && reference->is_object() && field(*reference, "player") != nullptr) {
    ...
    const std::size_t second =
        first == std::string::npos ? std::string::npos : id.find(':', first + 1);
    const std::string prefix = first == std::string::npos ? std::string{} : id.substr(0, first);
    if ((prefix != "pokemon" && prefix != "attachment") || second == std::string::npos)
        return fallback;
    try {
        return static_cast<std::int32_t>(std::stoll(id.substr(first + 1, second - first - 1)));
...
bool choice_option_is_hand_card(const Value &option) {
    ...
    if (reference != nullptr && reference->is_object()) {
        ...
        if (string_field(*reference, "kind") == "attachment")
            return false;
    }
    return true;
}
...
std::string choice_option_energy_card_id(const Value &option, const Value &cards) {
    ...
    const std::size_t target_separator = index_separator == std::string::npos
                                             ? std::string::npos
                                             : id.find("->", index_separator + 1);
    if (index_separator == std::string::npos || target_separator == std::string::npos ||
        target_separator <= index_separator + 1)
        return {};
    const std::string card_id =
        id.substr(index_separator + 1, target_separator - index_separator - 1);
...
std::string choice_score_mode(const Value &choice, std::int32_t actor) {
    ...
    if (request == "select_attachment") {
        if (purpose == "discard_energy" || purpose == "discard_energy_attachments") {
            return integer_field(presentation, "source_player", actor) == actor ? "energy_source"
                                                                                : "target";
        }
        return purpose.rfind("energy_relocate", 0) == 0 || purpose.rfind("relocate_energy", 0) == 0
                   ? "energy_source"
                   : "discard";
    }
    ...
    if (discard_purposes.count(purpose))
        return "discard";
    ...
    if (purpose == "discard" || purpose == "discard_cost" || purpose == "bottom_deck")
        return "discard";
    return "search";
}
    ...
    for (const std::string &attacker_id : attacker_ids) {
        ...
        for (const Value &attack : array_field(*definition, "attacks")) {
            for (const Value *effect : flatten_effects(array_field(attack, "effects"))) {
                if (string_field(*effect, "effect_type") == "damage_per_discard_psychic")
                    return true;
            }
        }
    }
...
double discard_choice_score(const RulesSession &position, const Value &state, std::int32_t actor,
    ...
    const Value &owner = player(state, actor);
    double score =
        -card_keep_value(position, state, actor, card_id, key, cards, decks, removing_from_hand);
    if (removing_from_hand) {
        const std::int64_t duplicates = count_in_zone(array_field(owner, "hand"), card_id);
        if (duplicates > 1)
            score += std::min(120.0, static_cast<double>(duplicates - 1) * 55.0);
        if (is_energy(cards, card_id) && bool_field(owner, "energy_attached_this_turn"))
            score += 35.0;
    ...
    if (discard_fuels_damage_plan(state, actor, card_id, cards))
        score += 76.0;
    return score;
}
...
double energy_choice_target_value(const RulesSession &position, const Value &state,
    ...
    const bool has_energy = !energy_card_id.empty() && is_energy(cards, energy_card_id);
    const std::int64_t before = best_missing(cards, pokemon);
    const Value probe = has_energy ? pokemon_with_extra_energy(*pokemon, energy_card_id) : *pokemon;
    ...
    const std::int64_t power_after =
        has_energy ? high_impact_missing_energy(position, state, actor, *pokemon, slot,
                                                energy_card_id, cards)
                   : power_before;
    ...
    const std::int64_t ready_after =
        best_ready_damage_for_pokemon(position, state, actor, probe, slot, cards);
    const DeckProfile deck = profile(cards, key);
    double value = static_cast<double>(progress) * 85.0;
    if (before > 0 && after == 0) {
        value +=
            ready_after <= 0 && power_after > 0 ? 55.0 : 155.0 + static_cast<double>(damage) * 0.25;
    } else if (before > 1 && after == 1)
        value += 65.0;
    if (damage >= deck.high_impact_damage_floor() && power_progress > 0) {
        value += static_cast<double>(power_progress) * 105.0;
        if (power_after == 0)
            value += 165.0 + static_cast<double>(damage) * 0.25;
        else if (power_after == 1)
            value += 115.0;
    }
    if (deck.contains("core", string_field(*pokemon, "card_id")))
        value += 65.0;
    value += energy_plan_target_bonus(position, state, actor, *pokemon, slot, energy_card_id, key,
                                      cards);
...
double energy_source_choice_value(const RulesSession &position, const Value &state,
    ...
    const std::int64_t after_high =
        high_impact_missing_energy(position, state, actor, probe, slot, {}, cards);
    ...
    const DeckProfile deck = profile(cards, key);
    if (damage >= deck.high_impact_damage_floor() && before_high == 0 && after_high > 0) {
    ...
    cost += static_cast<double>(std::max<std::int64_t>(0, before_ready - after_ready)) * 0.55;
    if (slot == "active") {
        cost += 80.0;
        if (before_ready > 0) {
    ...
    if (energy_unit_count(cards, pokemon) >= 3 && after_missing == 0 && after_ready >= before_ready)
        cost -= 100.0;
    else if (before_missing >= 2 && before_high >= 2)
        cost -= 45.0;
    return -cost;
}
...
double switch_opponent_attack_route_bonus(const RulesSession &position, const Value &state,
    ...
    try {
        bench_index = std::stoll(target_slot.substr(6));
    } catch (const std::exception &) {
        return 0.0;
    }
    ...
    *active_value = target;
    ...
    const std::int64_t damage = attacker == nullptr
                                    ? 0
                                    : best_ready_damage_for_pokemon(position, simulation, actor,
                                                                    *attacker, "active", cards);
    if (damage <= 0)
        return 0.0;
...
double target_choice_value(const RulesSession &position, const Value &state, std::int32_t actor,
    ...
    const std::int64_t amount = std::max<std::int64_t>(0, integer_field(presentation, "amount"));
    ...
    if (string_field(choice, "request_type") == "select_opponent_bench" ||
        ...
        value += switch_opponent_attack_route_bonus(position, state, actor, target_player, pokemon,
                                                    slot, cards);
    }
    if (amount <= 0)
        return value;
...
std::optional<double> base_choice_option_score(const RulesSession &position, const Value &state,
    ...
    if (mode == "discard")
        return discard_choice_score(position, state, actor, card_id, key, cards, decks,
    ...
    if (mode == "energy_source")
        return energy_source_choice_value(position, state, target_player, slot, presentation, key,
                                          cards);
    double score = mode == "search"
                       ? card_keep_value(position, state, actor, card_id, key, cards, decks)
                       : 0.0;
    const Value *pokemon = pokemon_at(player(state, target_player), slot);
    if (pokemon == nullptr)
        return score;
    if (mode == "target" || target_player != actor) {
        score += target_choice_value(position, state, actor, choice, target_player, *pokemon, slot,
                                     presentation, cards);
    } else if (mode == "heal") {
        ...
        if (energy_id.empty())
            energy_id = choice_energy_card_id(presentation, cards);
        score +=
            energy_choice_target_value(position, state, target_player, slot, energy_id, key, cards);
    } else if (mode == "self_switch") {
        score += promotion_value(position, state, target_player, *pokemon, slot, key, cards);
    } else {
...
std::vector<std::string> energy_plan_evolution_descendants(const Value &cards, const Value &decks,
    ...
    if (key.empty() || source_card_id.empty())
        return result;
    ...
    for (std::size_t depth = 0; depth < 2; ++depth) {
        ...
        for (const std::string &parent_id : frontier) {
            ...
            const std::string parent_name =
                parent == nullptr ? parent_id : string_field(*parent, "name", parent_id);
...
double energy_attack_plan_for_card(const RulesSession &position, const Value &state,
    ...
    for (std::size_t index = 0; index < attacks.size(); ++index) {
        ...
        const std::int64_t damage =
            estimated_damage_for_pokemon(position, state, actor, probe, slot, index, cards);
        double value = static_cast<double>(damage) - static_cast<double>(missing) * 50.0;
...
double energy_attack_plan_utility(const RulesSession &position, const Value &state,
    ...
    const Value *opponent_definition = opponent_active == nullptr
                                           ? nullptr
                                           : card(cards, string_field(*opponent_active, "card_id"));
    const std::int64_t opponent_prizes =
        opponent_definition == nullptr ? 0 : integer_field(*opponent_definition, "prize_value", 1);
    double best = energy_attack_plan_for_card(position, state, actor, pokemon, slot,
                                              string_field(pokemon, "card_id"), opponent_hp,
                                              opponent_prizes, cards);
    for (const std::string &descendant :
         energy_plan_evolution_descendants(cards, decks, string_field(pokemon, "card_id"), key)) {
        best = std::max(best, energy_attack_plan_for_card(position, state, actor, pokemon, slot,
                                                          descendant, opponent_hp, opponent_prizes,
                                                          cards) *
                                  0.75);
    }
    return best;
...
double public_energy_distribution_board_utility(const RulesSession &position, const Value &state,
    ...
    for (std::size_t board_index = 0; board_index < rows.size(); ++board_index) {
        ...
        for (const Value &attack : attacks) {
            const std::int64_t missing =
                missing_energy(cards, *pokemon, array_field(attack, "cost"));
            const std::int64_t damage = integer_field(attack, "damage");
            double value = static_cast<double>(damage) - static_cast<double>(missing) * 55.0;
            if (missing == 0 && damage > 0) {
                value += 80.0;
                if (damage >= profile(cards, key).high_impact_damage_floor())
                    value += 70.0;
                if (opponent_hp > 0 && damage >= opponent_hp) {
                    value += 200.0 + static_cast<double>(opponent_prizes) * 100.0;
                }
        ...
        if (std::isfinite(best)) {
            score += best * (board_index == 0 && active(state, actor) == pokemon ? 1.0 : 0.9);
        }
    }
    return score;
}

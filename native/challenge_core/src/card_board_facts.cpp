...
Value::Array expand_deck(const Value &decks, const std::string &key) {
    ...
    for (const Value &row : rows->as_array()) {
        ...
        for (std::int64_t copy = 0; copy < integer_field(row, "count"); ++copy)
...
std::vector<std::string> pre_evolution_ids(const Value &cards, const Value &decks,
    ...
    const std::string previous =
        definition == nullptr ? std::string{} : string_field(*definition, "evolves_from");
    std::vector<std::string> result;
    if (previous.empty() || key.empty())
        return result;
    for (const Value &candidate_value : expand_deck(decks, key)) {
        ...
        if (candidate_card != nullptr && is_pokemon(cards, candidate) &&
            string_field(*candidate_card, "name", candidate) == previous)
    ...
    if (is_stage2(cards, core)) {
        ...
        for (const std::string &stage1 : result.stage1) {
            for (const std::string &basic : pre_evolution_ids(cards, decks, stage1, key)) {
                if (!contains(result.basic, basic))
                    result.basic.push_back(basic);
            }
        }
...
bool zone_has(const Value::Array &zone, const std::vector<std::string> &ids) {
    return std::any_of(zone.begin(), zone.end(),
                       [&ids](const Value &entry) { return contains(ids, entry.string_or()); });
}

bool in_play(const Value &state, std::int32_t actor, const std::vector<std::string> &ids) {
    const auto rows = board(state, actor);
    return std::any_of(rows.begin(), rows.end(), [&ids](const Value *pokemon) {
        return contains(ids, string_field(*pokemon, "card_id"));
    });
}
...
std::string primary_core(const Value &cards, const Value &decks, const std::string &key) {
    for (const Value &core_value : profile(cards, key).cards("core")) {
        ...
        if (!is_pokemon(cards, core))
            continue;
        const LineParts parts = line_parts(cards, decks, core, key);
        if (!parts.stage1.empty() || !parts.basic.empty())
            return core;
    }
    return {};
}
...
double active_evolve_blocking_penalty(const RulesSession &position, const Value &state,
    ...
    if (primary.empty() || evolved_card_id == primary ||
    ...
    const std::int64_t evolved_ready =
        best_ready_damage_for_pokemon(position, state, actor, evolved, "active", cards);
    if (evolved_ready >= profile(cards, key).high_impact_damage_floor())
        return 0.0;

    const Value *before_definition = card(cards, string_field(evolve_target, "card_id"));
    const Value *after_definition = card(cards, evolved_card_id);
    if (before_definition == nullptr || after_definition == nullptr)
        return 0.0;
    const std::int64_t before_retreat = integer_field(*before_definition, "retreat_cost");
    const std::int64_t after_retreat = integer_field(*after_definition, "retreat_cost");
    const std::int64_t attached = energy_unit_count(cards, &evolve_target);
    double penalty = 0.0;
    if (after_retreat > before_retreat) {
        penalty += 115.0 + static_cast<double>(after_retreat - before_retreat) * 55.0;
    }
    if (after_retreat > attached)
        penalty += 90.0;

    const LineParts primary_parts = line_parts(cards, decks, primary, key);
    std::vector<std::string> primary_ids{primary};
    primary_ids.insert(primary_ids.end(), primary_parts.stage1.begin(), primary_parts.stage1.end());
    primary_ids.insert(primary_ids.end(), primary_parts.basic.begin(), primary_parts.basic.end());
    const Value &owner = player(state, actor);
    if (in_play(state, actor, primary_ids) || zone_has(array_field(owner, "hand"), primary_ids) ||
    ...
    for (std::size_t index = 0; index < bench.size(); ++index) {
        ...
        const std::int64_t ready =
            best_ready_damage_for_pokemon(position, state, actor, bench[index], slot, cards);
        const std::int64_t missing = best_missing(cards, &bench[index]);
        if (ready >= std::max<std::int64_t>(80, evolved_ready + 40) ||
    ...
    if (after_retreat >= 2 && evolved_ready <= 40)
        penalty += 70.0;
...
double promotion_value(const RulesSession &position, const Value &state, std::int32_t actor,
    ...
    const std::int64_t ready_damage =
        best_ready_damage_for_pokemon(position, state, actor, pokemon, slot, cards);
    ...
    double value = static_cast<double>(current_hp) + static_cast<double>(ready_damage) * 2.0 +
                   static_cast<double>(energy_count) * 35.0;
    const Value *opponent_active = active(state, 1 - actor);
    const bool can_take_prize =
        opponent_active != nullptr && ready_damage >= position.pokemon_current_hp(*opponent_active);
    if (missing == 0) {
        ...
        value += 55.0 + static_cast<double>(printed_best_damage(cards, &pokemon)) * 0.20;
    } else {
    ...
    if (players != nullptr && players->is_array() && actor >= 0 &&
        ...
        if (slot == "active") {
            opponent_damage = best_available_damage(position, simulation, 1 - actor, cards);
        }
        Value original_active =
            field(owner, "active") == nullptr ? Value() : *field(owner, "active");
        if (slot.rfind("bench_", 0) == 0) {
            ...
            if (bench != nullptr && bench->is_array() && index >= 0 &&
                static_cast<std::size_t>(index) < bench->as_array().size()) {
                bench->as_array()[static_cast<std::size_t>(index)] = std::move(original_active);
    ...
    if (survives) {
        ...
        const double asset =
            card_priority(cards, string_field(pokemon, "card_id"), key) +
            static_cast<double>(energy_count) * 45.0 +
            static_cast<double>(array_field(pokemon, "evolution_stack_ids").size()) * 45.0 +
            static_cast<double>(
                definition == nullptr ? 1 : integer_field(*definition, "prize_value", 1)) *
                120.0;
    ...
    if (opponent_damage >= current_hp) {
        ...
        if (!can_take_prize) {
            ...
            if (profile(cards, key).contains("core", card_id))
                value -= 85.0;
            if (profile(cards, key).contains("engine", card_id))
                value -= 45.0;
        }
    }
    if (missing > 0 && profile(cards, key).contains("bench", string_field(pokemon, "card_id")) &&
        !profile(cards, key).contains("setup", string_field(pokemon, "card_id"))) {
        value -= 70.0 + std::min(80.0, static_cast<double>(missing) * 24.0);
    }
    return value;
}
...
bool redundant_same_pokemon_retreat(const RulesSession &position, const Value &state,
    ...
    if (string_field(target, "card_id") != string_field(active_pokemon, "card_id"))
        return false;
    ...
    if (position.pokemon_current_hp(target) > position.pokemon_current_hp(active_pokemon))
        return false;
    const std::int64_t active_ready =
        best_ready_damage_for_pokemon(position, state, actor, active_pokemon, "active", cards);
    const std::int64_t target_ready =
        best_ready_damage_for_pokemon(position, state, actor, target, target_slot, cards);
    if (target_ready > active_ready)
        return false;
    const double active_value =
        promotion_value(position, state, actor, active_pokemon, "active", key, cards);
    const double target_value =
        promotion_value(position, state, actor, target, target_slot, key, cards);
...
bool retreat_has_good_target(const RulesSession &position, const Value &state, std::int32_t actor,
                             ...
                             const Value &cards) {
    if (redundant_same_pokemon_retreat(position, state, actor, active_pokemon, target, target_slot,
                                       key, cards))
        return false;
    const Value *opponent_active = active(state, 1 - actor);
    const std::int64_t target_ready =
    ...
    const bool active_survives = opponent_damage < position.pokemon_current_hp(active_pokemon);
    const bool target_falls = opponent_damage >= position.pokemon_current_hp(target);
    if (active_survives && target_falls)
        return false;
    ...
    const bool active_safe = array_field(active_pokemon, "status_conditions").empty() &&
...
double focus_multiplier(const Value &state, std::int32_t actor, const Value &cards,
    ...
    const std::string primary = primary_core(cards, decks, key);
    if (primary.empty() || core == primary)
        return 1.0;
    if (in_play(state, actor, {primary}))
        return 0.72;
    ...
    if (in_play(state, actor, ids) || zone_has(array_field(owner, "hand"), ids) ||
        zone_has(array_field(owner, "deck"), ids))
        return 0.42;
    return 0.65;
}
...
double core_line_value(const Value &state, std::int32_t actor, const Value &cards,
    ...
    for (const Value &core_value : profile(cards, key).cards("core")) {
        ...
        if (definition != nullptr && integer_field(*definition, "prize_value", 1) >= 2)
            line *= 1.1;
...
double active_ko_risk(const RulesSession &position, const Value &state, std::int32_t actor,
    ...
    if (opponent_damage < hp)
        return static_cast<double>(opponent_damage) >= static_cast<double>(hp) * 0.65 ? 62.0 : 0.0;
    ...
    if (profile(cards, key).contains("core", string_field(*own_active, "card_id")))
        risk += 70.0;
    return risk;
}
...
double resource_outs(const Value &state, std::int32_t actor, const Value &cards, const Value &decks,
    ...
    if (has_energy_target(state, actor, cards)) {
        ...
        for (const Value &entry : array_field(owner, "hand")) {
            ...
            if (is_energy(cards, id) && (key.empty() || energy_matches_profile(cards, id, key))) {
                ++hand_energy;
            }
        }
        ...
        value += std::min(110.0, static_cast<double>(hand_energy) * 42.0 +
                                     std::min(54.0, static_cast<double>(deck_energy) * 9.0));
    }
    double evolution_value = 0.0;
    for (const Value *pokemon : board(state, actor)) {
        if (!bool_field(*pokemon, "can_evolve_this_turn", true))
            continue;
        ...
        const bool in_hand = std::any_of(array_field(owner, "hand").begin(),
                                         array_field(owner, "hand").end(), matches_evolution);
        const bool in_deck =
            !in_hand && std::any_of(array_field(owner, "deck").begin(),
                                    array_field(owner, "deck").end(), matches_evolution);
        if (in_hand) {
    ...
    for (const Value &entry : array_field(owner, "hand")) {
        ...
        } else if (bench_space && is_basic_pokemon(cards, id) && priority > 0.0) {
...
std::int64_t count_in_play(const Value &state, std::int32_t actor,
    ...
    std::int64_t count = 0;
    for (const Value *pokemon : board(state, actor)) {
        if (contains(ids, string_field(*pokemon, "card_id")))
            ++count;
    }
    return count;
}
...
std::int64_t count_in_zone(const Value::Array &zone, const std::string &card_id) {
    return static_cast<std::int64_t>(
        std::count_if(zone.begin(), zone.end(),
...
double core_evolution_line_card_bonus(const Value &state, std::int32_t actor,
    ...
    if (key.empty() || !is_pokemon(cards, card_id))
        return 0.0;
    const Value &owner = player(state, actor);
    double best = 0.0;
    for (const Value &core_value : profile(cards, key).cards("core")) {
        const std::string core = core_value.string_or();
        if (core.empty() || !is_pokemon(cards, core))
            continue;
        const LineParts parts = line_parts(cards, decks, core, key);
        if (parts.stage1.empty() && parts.basic.empty())
            continue;
        ...
        const bool has_stage1 = in_play(state, actor, parts.stage1);
        const bool has_basic = in_play(state, actor, parts.basic);
        const bool has_rare_candy = array_contains(array_field(owner, "hand"), "sv1-152");
        const std::int64_t stage1_count = count_in_play(state, actor, parts.stage1);
        const std::int64_t basic_count = count_in_play(state, actor, parts.basic);
        const std::int64_t receivers = is_stage2(cards, core)
                                           ? stage1_count + (has_rare_candy ? basic_count : 0)
                                           : basic_count;
        const std::int64_t core_supply =
            std::max<std::int64_t>(0, count_in_zone(array_field(owner, "hand"), core) -
                                          (removing_one && card_id == core ? 1 : 0));
        double bonus = 0.0;
        if (card_id == core) {
            const bool surplus = removing_one ? core_supply >= receivers : core_supply > receivers;
            if (surplus)
                bonus = -180.0;
            else if (is_stage2(cards, core)) {
                if (has_stage1)
                    bonus = 170.0;
                else if (has_basic && has_rare_candy)
                    bonus = 70.0;
                else if (has_basic)
                    bonus = -35.0;
                else if (!has_core)
            ...
            const std::int64_t supply = std::max<std::int64_t>(
                0, count_in_zone(array_field(owner, "hand"), card_id) - (removing_one ? 1 : 0));
            const bool surplus = removing_one ? supply >= basic_count : supply > basic_count;
            if (surplus)
                bonus = -120.0;
            else if (has_basic)
                bonus = 145.0;
            else if (!has_stage1 && !has_core)
                bonus = 25.0;
        } else if (contains(parts.basic, card_id)) {
            const std::int64_t supply_after = std::max<std::int64_t>(
                0, count_in_zone(array_field(owner, "hand"), card_id) - (removing_one ? 1 : 0));
            if (removing_one && supply_after >= 1 && !has_basic && !has_stage1 && !has_core)
                bonus = -80.0;
            else if (!has_basic && !has_stage1 && !has_core)
                bonus = 155.0;
            else if (has_basic && !has_stage1 && !has_core)
                bonus = 55.0;
        }
...
bool energy_improves_attack_readiness(const Value &state, std::int32_t actor,
    ...
    if (!is_energy(cards, energy_card_id))
        return false;
    for (const Value *pokemon : board(state, actor)) {
        const Value *definition = card(cards, string_field(*pokemon, "card_id"));
        if (definition == nullptr)
        ...
        for (const Value &attack : array_field(*definition, "attacks")) {
            ...
            if (before > 0 && missing_energy(cards, probe, cost) < before)
                return true;
        }
    }
    return false;
...
std::int64_t helpful_hand_energy_count(const Value &state, std::int32_t actor, const Value &cards) {
    ...
    for (const Value &entry : array_field(player(state, actor), "hand")) {
        if (energy_improves_attack_readiness(state, actor, entry.string_or(), cards))
            ++result;
    }
    return result;
}
...
double lone_active_backup_search_bonus(const RulesSession &position, const Value &state,
    ...
    const Value *own_active = active(state, actor);
    if (own_active == nullptr || bench_count(player(state, actor)) > 0)
        return 0.0;
    const Value *opponent_active = active(state, 1 - actor);
    if (opponent_active == nullptr)
        return 80.0;
    const std::int64_t hp = position.pokemon_current_hp(*own_active);
    const std::int64_t ready = best_available_damage(position, state, 1 - actor, cards);
...
bool has_core_basic_out_in_deck(const Value &state, std::int32_t actor, const std::string &key,
    ...
    for (const Value &entry : array_field(player(state, actor), "deck")) {
        ...
        if (is_basic_pokemon(cards, id) &&
            (deck.contains("setup", id) || deck.contains("bench", id) || deck.contains("core", id)))
            return true;
    }
    return false;
}
...
double bench_setup_search_card_bonus(const RulesSession &position, const Value &state,
                                     ...
                                     const std::string &key, const Value &cards) {
    const Value &owner = player(state, actor);
    if (bench_count(owner) >= 3)
        return 0.0;
    const Value *definition = card(cards, card_id);
    if (definition == nullptr)
        return 0.0;
    bool finds_basic = false;
    for (const Value *effect : flatten_effects(array_field(*definition, "trainer_effects"))) {
        const Value *params = field(*effect, "params");
        if (string_field(*effect, "effect_type") == "search" && params != nullptr &&
            params->is_object() && string_field(*params, "destination") == "bench" &&
    ...
    for (const Value &entry : array_field(owner, "deck")) {
        if (is_basic_pokemon(cards, entry.string_or()))
            ++basic_outs;
    }
    if (basic_outs <= 0)
        return 0.0;
    double value = 55.0 + std::min(80.0, static_cast<double>(basic_outs) * 10.0);
    if (bench_count(owner) == 0)
        value += 150.0;
    else if (bench_count(owner) == 1)
        value += 75.0;
    const Value *own_active = active(state, actor);
    if (own_active != nullptr && best_available_damage(position, state, 1 - actor, cards) >=
                                     position.pokemon_current_hp(*own_active))
        value += 95.0;
    if (has_core_basic_out_in_deck(state, actor, key, cards))
        value += 55.0;
    return value;
}

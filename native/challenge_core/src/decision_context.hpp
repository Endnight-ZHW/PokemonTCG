...
class DecisionContext {
  public:
    ...
    void set_incumbent(const Value &action) {
        if (!(incumbent_action_ == action)) {
            incumbent_action_ = action;
            ++ranking_epoch_;
        }
    }
    std::uint64_t ranking_policy() const noexcept {
        return ranking_epoch_ * 2 + (shortlist_enabled ? 1 : 0);
    }
    std::atomic<std::uint64_t> rule_actions{0}, rule_choices{0};
    std::atomic<std::uint64_t> rejected_rule_actions{0}, rejected_rule_choices{0};
    std::atomic<std::uint64_t> ranked_input_actions{0}, detailed_action_scores{0};
    std::atomic<std::uint64_t> completed_comparisons{0};
    std::size_t worker_count = 1;

    template <class Compute>
    std::unique_ptr<RulesSession> sample(std::size_t sample_index, std::uint32_t seed,
                                         Compute &&compute) {
    ...
    T memoize(SearchMemo kind, const RulesSession &position, const Value &state, std::int32_t actor,
              std::uint64_t policy, Compute &&compute) {
        return memoize_values<T>(kind, position.search_state(), state, position.rng_state(), actor,
                                 policy, std::forward<Compute>(compute));
    }

    template <class T, class Compute>
    T memoize_values(SearchMemo kind, const Value &basis, const Value &state, std::uint32_t rng,
        ...
        if (!memoization_enabled || !identity(state) || !identity(basis))
            return compute();
        const Key key{identity(state), identity(basis), rng, actor, policy, kind};
        ...
        ++misses_[index];
        T result = compute();
        if (budget && budget->status() != EvaluationStatus::Complete)
            return result;
        {
            std::lock_guard<std::mutex> lock(mutex_);
    ...
    Value counters() const {
        static constexpr const char *names[] = {"pipeline",    "resources",   "zones",
                                                "state_score", "fingerprint", "preconditions",
                                                "ranking"};
        ...
        result["rejected_rule_actions"] =
            Value(static_cast<std::int64_t>(rejected_rule_actions.load()));
        result["rejected_rule_choices"] =
            Value(static_cast<std::int64_t>(rejected_rule_choices.load()));
        result["ranked_input_actions"] =
            Value(static_cast<std::int64_t>(ranked_input_actions.load()));
        result["detailed_action_scores"] =
            Value(static_cast<std::int64_t>(detailed_action_scores.load()));
        result["sample_cache_hits"] = Value(static_cast<std::int64_t>(sample_hits_.load()));
        result["completed_plan_comparisons"] =
            Value(static_cast<std::int64_t>(completed_comparisons.load()));
        if (budget) {
            const auto status = budget->status();
            result["budget_status"] = Value(status == EvaluationStatus::Complete    ? "available"
                                            : status == EvaluationStatus::Cancelled ? "cancelled"
                                                                                    : "exhausted");
            ...
            result["interrupted_evaluations"] =
                Value(static_cast<std::int64_t>(budget->interrupted_evaluations()));
        }
  ...
  private:
    ...
    struct Hash {
        std::size_t operator()(const Key &key) const noexcept {
            ...
            mix(key.rng);
            mix(static_cast<std::size_t>(key.actor));

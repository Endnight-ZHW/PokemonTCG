...
bool ChallengeSearchProviderImpl::forced_choice_response(const ptcg::ai::Value &state,
    ...
    if (choice.options.empty()) {
    ...
    } else if (choice.options.size() == 1 && choice.min_select == 1 && choice.max_select == 1 &&
        ...
        if (choice.request_kind == Kind::choose_turn_order) {
            const auto *keys = state.find("public_deck_keys");
            const std::string key =
                keys && keys->is_array() && choice.player >= 0 &&
                        static_cast<std::size_t>(choice.player) < keys->as_array().size()
                    ? keys->as_array()[choice.player].string_or()
                    : "";
            selected_ids.emplace_back(
                strategy_catalog_.policy_for(key).prefer_first() ? "turn:first" : "turn:second");
        } else if (choice.request_kind == Kind::choose_mulligan_draw_count) {
            std::int64_t largest_draw = -1;
            for (const ptcg::ai::typed::ChoiceOption &option : choice.options) {
            ...
            selected_ids.emplace_back("draw:" +
                                      std::to_string(std::max<std::int64_t>(0, largest_draw)));
            ...
            selected_ids.emplace_back("prize:" +
                                      std::to_string(lowest_prize == 999 ? 0 : lowest_prize));
            ...
            const ptcg::ai::Value &view =
                presentation != nullptr && presentation->is_object() ? *presentation : empty;
            const std::string purpose = string_field(view, "purpose");
            if (purpose == "trekking_shoes" || !string_field(view, "top_card_id").empty() ||
                purpose == "confirm_switch" || purpose == "search_any_switch_confirm" ||
                purpose == "switch") {
                return false;
            }
            bool confirmed = true;
            ...
            selected_ids.emplace_back(confirmed ? "confirm:yes" : "confirm:no");
        } else {
            return false;
        }
    }

...
inline bool has_subtype(const Value &definition, const std::string &subtype) {
    return traditional_value::array_contains(traditional_value::array_field(definition, "subtypes"),
                                             subtype);
}
...
inline bool is_pokemon(const Value &cards, const std::string &card_id) {
    ...
    return definition != nullptr &&
           traditional_value::string_field(*definition, "supertype") == "Pokémon";
}
...
inline bool is_trainer(const Value &cards, const std::string &card_id) {
    const Value *definition = card(cards, card_id);
    return definition != nullptr &&
           traditional_value::string_field(*definition, "supertype") == "Trainer";
}

inline bool is_energy(const Value &cards, const std::string &card_id) {
    const Value *definition = card(cards, card_id);
    return definition != nullptr &&
           traditional_value::string_field(*definition, "supertype") == "Energy";
}
...
inline bool downgrades_rainbow(const Value &definition) {
    for (const Value &effect : traditional_value::array_field(definition, "energy_effects")) {
        if (traditional_value::string_field(effect, "kind") == "provide_energy" &&
            traditional_value::bool_field(effect, "downgrade_if_other_special")) {
            return true;
        }
    }
    return false;
...
inline std::vector<std::string> energy_units(const Value &cards, const Value &pokemon) {
    std::vector<std::string> result;
    const Value::Array &attached = traditional_value::array_field(pokemon, "energy_card_ids");
    for (std::size_t index = 0; index < attached.size(); ++index) {
        ...
        if (downgrades_rainbow(*definition)) {
            for (std::size_t other = 0; other < attached.size(); ++other) {
                if (other != index && is_special_energy(cards, attached[other].string_or())) {
                    downgrade = true;
                    break;
                }
            }
        }
        for (const Value &provided :
             traditional_value::array_field(*definition, "provides_energy")) {
            const std::string unit = provided.string_or();
            result.push_back(downgrade && unit == "Rainbow" ? "Colorless" : unit);
        }
    }
    return result;
}

inline std::int64_t energy_unit_count(const Value &cards, const Value *pokemon) {
    return pokemon == nullptr ? 0 : static_cast<std::int64_t>(energy_units(cards, *pokemon).size());
}

inline std::int64_t energy_type_count(const Value &cards, const Value *pokemon,
    ...
    return static_cast<std::int64_t>(
        std::count_if(units.begin(), units.end(), [&energy_type](const std::string &unit) {
            const std::string normalized = traditional_value::lower_ascii(unit);
            return normalized == energy_type || normalized == "rainbow";
        }));
}

inline std::int64_t missing_energy_units(std::vector<std::string> available,
    ...
    return missing +
           std::max<std::int64_t>(0, colorless - static_cast<std::int64_t>(available.size()));
}

inline std::int64_t missing_energy(const Value &cards, const Value &pokemon,
                                   const Value::Array &cost) {
...
inline std::int64_t bench_count(const Value &owner) {
    ...
    return static_cast<std::int64_t>(std::count_if(
        bench.begin(), bench.end(), [](const Value &pokemon) { return pokemon.is_object(); }));
}

...
bool ChallengeSearchProviderImpl::confirm_choice_response(const ptcg::ai::RulesSession &position,
    ...
    const std::optional<bool> confirmed =
        card_evaluator_.confirm_choice(position, choice.player, pending);
...
std::string
ChallengeSearchProviderImpl::resolved_option_card_id(const ptcg::ai::Value &option) const {
...
double ChallengeSearchProviderImpl::prize_aware_search_choice_score(
    const ptcg::ai::RulesSession &position, std::int32_t actor, const ptcg::ai::Value &pending,
    const ptcg::ai::Value &option) const {
    if (actor != root_actor_ || information_set_ == nullptr || !information_set_->valid() ||
    ...
    if (presentation == nullptr || !presentation->is_object() ||
        ...
        string_field(*presentation, "source_zone") != "deck" || reference == nullptr ||
        !reference->is_object() || string_field(*reference, "kind") != "card" ||
    ...
    const ptcg::ai::Value &owner = players->as_array()[static_cast<std::size_t>(actor)];
    const auto count_zone = [&card_id](const ptcg::ai::Value *zone) {
        ...
        return static_cast<std::int64_t>(std::count_if(
            ...
            [&card_id](const ptcg::ai::Value &entry) { return entry.string_or() == card_id; }));
    };
    const std::int64_t deck_copies = count_zone(owner.find("deck"));
    if (deck_copies <= 0)
        return 0.0;
    const std::int64_t prize_copies = static_cast<std::int64_t>(std::count_if(
        information_set_->known_prizes(actor).begin(), information_set_->known_prizes(actor).end(),
        [&card_id](const ptcg::ai::Value &entry) { return entry.string_or() == card_id; }));

    double dead_evolution_line_penalty = 0.0;
    const ptcg::ai::Value *definition = std::as_const(cards_).find(card_id);
    const auto has_role = [&](const std::string &candidate_id, const char *name) {
        return strategy_catalog_.card_has_role(state, actor, candidate_id, name);
    };
    const bool candidate_is_attacker =
        has_role(card_id, "primary_attacker") || has_role(card_id, "secondary_attacker");
    if (definition != nullptr && definition->is_object() &&
        string_field(*definition, "supertype") == "Pokémon" && !candidate_is_attacker) {
        const std::string candidate_name = string_field(*definition, "name");
        std::set<std::string> direct_names;
        std::set<std::string> evolution_ids;
        for (const auto &[evolution_id, evolution] : cards_.as_object()) {
            if (!evolution.is_object() || string_field(evolution, "evolves_from") != candidate_name)
                continue;
        ...
        if (!direct_names.empty()) {
            for (const auto &[evolution_id, evolution] : cards_.as_object()) {
                if (!evolution.is_object() ||
                    direct_names.count(string_field(evolution, "evolves_from")) == 0)
                    continue;
                if (has_role(evolution_id, "primary_attacker") ||
                    has_role(evolution_id, "secondary_attacker") ||
                    has_role(evolution_id, "bench_engine") ||
        ...
        if (!evolution_ids.empty()) {
            const auto count_ids = [&evolution_ids](const ptcg::ai::Value *zone) {
                if (zone == nullptr || !zone->is_array()) {
                    return std::int64_t{0};
                }
                return static_cast<std::int64_t>(
                    std::count_if(zone->as_array().begin(), zone->as_array().end(),
                                  [&evolution_ids](const ptcg::ai::Value &entry) {
                                      return evolution_ids.count(entry.string_or()) != 0;
                                  }));
            };
            const auto count_known = [&evolution_ids](const std::vector<ptcg::ai::Value> &zone) {
                return static_cast<std::int64_t>(std::count_if(
                    zone.begin(), zone.end(), [&evolution_ids](const ptcg::ai::Value &entry) {
                        return evolution_ids.count(entry.string_or()) != 0;
                    }));
            };
            std::int64_t accessible = count_known(information_set_->known_deck(actor));
            ...
            const auto count_pokemon_id = [&](const ptcg::ai::Value *pokemon) {
                if (pokemon != nullptr && pokemon->is_object() &&
                    evolution_ids.count(string_field(*pokemon, "card_id")) != 0)
                    ++accessible;
            };
            ...
            const std::int64_t prized = count_known(information_set_->known_prizes(actor));
    ...
    const auto role = [&](const char *name) {
        return strategy_catalog_.card_has_role(state, actor, card_id, name);
    };
    if (role("primary_attacker"))
        role_value = std::max(role_value, 22.0);
    if (role("bench_engine") || role("energy_acceleration")) {
        role_value = std::max(role_value, 19.0);
    }
    if (role("setup_basic"))
        role_value = std::max(role_value, 15.0);
    if (role("evolution"))
        role_value = std::max(role_value, 14.0);
    if (role("secondary_attacker"))
        role_value = std::max(role_value, 11.0);
    if (role("recovery"))
        role_value = std::max(role_value, 8.0);
    if (role("search"))
        role_value = std::max(role_value, 6.0);
    if (role("energy"))
        role_value = std::max(role_value, 5.0);
    double scarcity_bonus = 0.0;
    if (role_value > 0.0 && prize_copies > 0) {
        double scarcity = deck_copies == 1 ? 1.0 : (deck_copies == 2 ? 0.35 : 0.0);
        if (scarcity > 0.0) {
            scarcity += std::min(0.5, static_cast<double>(prize_copies) * 0.2);
            ...
            const auto count_pokemon = [&](const ptcg::ai::Value *pokemon) {
                ...
                const ptcg::ai::Value *stack = pokemon->find("evolution_stack_ids");
            ...
            if (visible_copies > 0)
                scarcity *= 0.35;
            scarcity_bonus = std::clamp(role_value * scarcity, 0.0, 30.0);
        }
    }
    const double result = std::clamp(dead_evolution_line_penalty + scarcity_bonus, -180.0, 30.0);
    if (std::abs(result) > 0.000001) {
        prize_aware_choice_adjustments_.fetch_add(1, std::memory_order_relaxed);
    }
    return result;
}

bool ChallengeSearchProviderImpl::arven_choice_response(const ptcg::ai::RulesSession &position,
                                                        const ptcg::ai::Value &pending,
                                                        const ptcg::ai::typed::ChoiceView &choice,
                                                        ptcg::ai::Value &response) const {
    if (choice.request_type != "arven")
        return false;
    const ptcg::ai::Value *options_value = pending.find("options");
    if (options_value == nullptr || !options_value->is_array() || options_value->as_array().empty())
        return false;
    const auto &options = options_value->as_array();
    const auto has_subtype = [this](const std::string &card_id, const std::string &subtype) {
        const ptcg::ai::Value *definition = std::as_const(cards_).find(card_id);
        const ptcg::ai::Value *subtypes = definition != nullptr && definition->is_object()
    ...
    for (std::size_t index = 0; index < options.size(); ++index) {
        ...
        const double score =
            card_evaluator_.choice_option_score(position, choice.player, pending, options[index])
                .value_or(0.0) +
            strategy_catalog_.choice_score(position.search_state(), choice.player, pending,
                                           options[index]) +
    ...
    if (best_item >= 0)
        selected.emplace_back(
            string_field(options[static_cast<std::size_t>(best_item)], "option_id"));
    if (best_tool >= 0 &&
        selected.size() < static_cast<std::size_t>(std::max<std::int64_t>(0, choice.max_select))) {
        selected.emplace_back(
            string_field(options[static_cast<std::size_t>(best_tool)], "option_id"));
    }
    if (selected.empty() && choice.min_select > 0) {
        std::size_t fallback = 0;
        double fallback_score =
            card_evaluator_.choice_option_score(position, choice.player, pending, options[0])
                .value_or(0.0);
        for (std::size_t index = 1; index < options.size(); ++index) {
            const double score =
                card_evaluator_
                    .choice_option_score(position, choice.player, pending, options[index])
                    .value_or(0.0);
...
bool ChallengeSearchProviderImpl::sequential_discard_response(
    const ptcg::ai::RulesSession &position, const ptcg::ai::Value &pending,
    const ptcg::ai::typed::ChoiceView &choice, ptcg::ai::Value &response) const {
    const ptcg::ai::Value *options_value = pending.find("options");
    if (options_value == nullptr || !options_value->is_array())
        return false;
    const auto &options = options_value->as_array();
    const std::size_t maximum = static_cast<std::size_t>(std::max<std::int64_t>(
        0, std::min<std::int64_t>(choice.max_select, static_cast<std::int64_t>(options.size()))));
    if (maximum <= 1 || choice.allow_duplicates)
        return false;
    ...
    const auto approximately_equal = [](double left, double right) {
        const double tolerance = 0.00001 * std::max(1.0, std::max(std::abs(left), std::abs(right)));
        return std::abs(left - right) <= tolerance;
    };
    for (std::size_t selection_index = 0; selection_index < maximum; ++selection_index) {
        std::unique_ptr<ptcg::ai::RulesSession> virtual_position =
            position.fork_for_search(position.rng_state());
        std::string restore_error;
        if (!virtual_position ||
            !virtual_position->restore(virtual_state, position.rng_state(), &restore_error))
            return false;
        std::int64_t best_index = -1;
        double best_score = -std::numeric_limits<double>::infinity();
        std::string best_tiebreak;
        for (std::size_t option_index = 0; option_index < options.size(); ++option_index) {
            if (std::find(selected_indices.begin(), selected_indices.end(), option_index) !=
                selected_indices.end())
                continue;
            const ptcg::ai::Value &option = options[option_index];
            const std::optional<double> base = card_evaluator_.choice_option_score(
            ...
            const std::string tiebreak = card_id + "|" + string_field(option, "option_id");
            if (best_index < 0 || score > best_score + 0.001 ||
                (approximately_equal(score, best_score) && tiebreak < best_tiebreak)) {
        ...
        if (best_index < 0)
            return false;
        if (selection_index >=
                static_cast<std::size_t>(std::max<std::int64_t>(0, choice.min_select)) &&
            best_score <= 0.0)
            break;
        ...
        if (is_hand_card(option)) {
            ...
            if (players == nullptr || !players->is_array() || choice.player < 0 ||
                ...
                return false;
            ptcg::ai::Value &owner = players->as_array()[static_cast<std::size_t>(choice.player)];
...
bool ChallengeSearchProviderImpl::single_choice_response(const ptcg::ai::RulesSession &position,
                                                         const ptcg::ai::Value &pending,
                                                         const ptcg::ai::typed::ChoiceView &choice,
                                                         ptcg::ai::Value &response) const {
    using Kind = ptcg::ai::typed::ChoiceRequestKind;
    if (choice.options.empty() || choice.allow_duplicates || choice.request_kind == Kind::confirm ||
        ...
        choice.request_kind == Kind::select_retreat_payment || choice.request_type == "arven")
        return false;
    const ptcg::ai::Value *options_value = pending.find("options");
    if (options_value == nullptr || !options_value->is_array() ||
    ...
    static const ptcg::ai::Value empty_presentation = ptcg::ai::Value::make_object();
    const ptcg::ai::Value &presentation_for_mode =
        presentation_probe != nullptr && presentation_probe->is_object() ? *presentation_probe
                                                                         : empty_presentation;
    const std::string request_type = string_field(pending, "request_type");
    const std::string purpose = string_field(presentation_for_mode, "purpose");
    const bool attachment_discard =
        request_type == "select_attachment" && purpose != "discard_energy" &&
        purpose != "discard_energy_attachments" && purpose.rfind("energy_relocate", 0) != 0 &&
    ...
    const bool discard_purpose = purpose == "discard_then_draw" ||
                                 ...
                                 purpose == "houb" || purpose == "zinnia" || purpose == "discard" ||
                                 purpose == "discard_cost" || purpose == "bottom_deck";
    if (choice.max_select > 1 && (attachment_discard || discard_purpose)) {
        return sequential_discard_response(position, pending, choice, response);
    }
    ...
    for (std::size_t index = 0; index < choice.options.size(); ++index) {
        ...
        const std::optional<double> base =
            card_evaluator_.choice_option_score(position, actor, pending, option);
        if (!base.has_value())
            return false;
        ranked.push_back({
            index,
            *base +
                strategy_catalog_.choice_score(position.search_state(), actor, pending, option) +
                prize_aware_search_choice_score(position, actor, pending, option),
        });
    }
    const auto approximately_equal = [](double left, double right) {
        const double tolerance = 0.00001 * std::max(1.0, std::max(std::abs(left), std::abs(right)));
    ...
    std::stable_sort(ranked.begin(), ranked.end(),
                     [&approximately_equal](const ScoredOption &left, const ScoredOption &right) {
    ...
    const ptcg::ai::Value &presentation =
        presentation_value != nullptr && presentation_value->is_object() ? *presentation_value
                                                                         : empty;
    const std::int64_t max_per_target =
        std::max<std::int64_t>(0, integer_field(presentation, "max_per_target", 2147483647));
    const ptcg::ai::Value *category_limits = presentation.find("category_limits");
    const auto category_for = [this](const ptcg::ai::Value &option) {
        const ptcg::ai::Value *reference = option.find("ref");
        const std::string card_id = reference != nullptr && reference->is_object()
                                        ? string_field(*reference, "card_id")
                                        : std::string{};
        if (card_id.empty())
            return std::string{};
        const ptcg::ai::Value *definition = std::as_const(cards_).find(card_id);
        ...
        const auto has_subtype = [&subtypes](const std::string &needle) {
            ...
            return std::any_of(
                ...
                [&needle](const ptcg::ai::Value &entry) { return entry.string_or() == needle; });
        };
        if (supertype == "Pokémon")
            return std::string("pokemon");
        if (supertype == "Energy")
        ...
        if (supertype == "Trainer")
            return std::string("trainer");
        return std::string{};
    };
    const auto category_limit = [&](const std::string &category) {
        if (category.empty())
            return std::int64_t{2147483647};
        ...
        const ptcg::ai::Value *count_field = presentation.find(category + "_count");
    ...
    const auto target_key_for = [](const ptcg::ai::Value &option) {
        ...
        if (reference != nullptr && reference->is_object()) {
            ...
            if (!slot.empty()) {
                return std::to_string(integer_field(*reference, "player", -1)) + ":" + slot;
            }
        }
        return "option:" + string_field(option, "option_id");
    };
    const std::size_t maximum = static_cast<std::size_t>(std::max<std::int64_t>(
        0, std::min<std::int64_t>(choice.max_select,
                                  static_cast<std::int64_t>(choice.options.size()))));
    std::size_t desired_count = maximum;
    if (choice.min_select <= 0) {
        desired_count =
            std::min(maximum, static_cast<std::size_t>(std::count_if(
    ...
    if (desired_count > 0 && max_per_target > 0) {
        ...
        for (const ScoredOption &row : ranked) {
            ...
            const std::string option_id = string_field(option, "option_id");
            if (option_id.empty() || used_option_ids.count(option_id))
                continue;
            const std::string target_key = target_key_for(option);
            if (same_target && !selected_target.empty() && target_key != selected_target)
                continue;
            if (per_target[target_key] >= max_per_target)
                continue;
            const std::string category = category_for(option);
            if (per_category[category] >= category_limit(category))
                continue;
            selected_ids.emplace_back(option_id);
            ...
            if (selected_ids.size() >= desired_count)
                break;
        }
    }
    const bool cancelled = selected_ids.empty() && choice.min_select <= 0 && choice.can_cancel;
    response = ptcg::ai::Value(ptcg::ai::Value::Object{
        {"request_id", ptcg::ai::Value(choice.request_id)},
        {"option_ids", ptcg::ai::Value(std::move(selected_ids))},
        {"cancelled", ptcg::ai::Value(cancelled)},
    });
    return choice.min_select <= 0 || !response.find("option_ids")->as_array().empty();
}

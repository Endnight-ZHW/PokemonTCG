...
class DecisionBudget {
  public:
    ...
    DecisionBudget(TimePoint started, std::int64_t milliseconds,
        ...
        : started_(started), duration_ms_(std::max<std::int64_t>(0, milliseconds)), cancel_(cancel),
          now_(std::move(now)), phase_started_(started_) {
    ...
    const char *last_stop_reason() const noexcept {
        static constexpr const char *reasons[] = {"none", "initial_limit", "search_limit",
                                                  "hard_deadline", "cancelled"};
    ...
    double remaining_ms() const {
        if (!timed())
            return 0;
        return std::max(
            0.0, std::chrono::duration<double, std::milli>(search_deadline_ - now_()).count());
    }
    ...
    auto evaluate(Compute &&compute) -> SearchEvaluation<decltype(compute())> {
        ...
        if (before != EvaluationStatus::Complete)
            return {before, std::nullopt};
        T result = compute();
...
class DecisionPhaseScope {
  public:
    DecisionPhaseScope(DecisionBudget *budget, DecisionPhase phase)
        ...
        if (budget_)
            budget_->set_phase(phase);
    }
    ~DecisionPhaseScope() {
        if (budget_)
            budget_->set_phase(previous_);
    }

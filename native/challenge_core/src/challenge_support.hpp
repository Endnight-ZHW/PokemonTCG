...
inline std::string value_string_field(const ptcg::ai::Value &value, const char *key,
                                      std::string fallback = {}) {
    const ptcg::ai::Value *entry = value.find(key);
    return entry == nullptr ? std::move(fallback) : entry->string_or(std::move(fallback));
}
    ...
    if (value.type() == Type::object) {
        ...
        for (const auto &[key, nested] : value.as_object()) {
            if (!first)
                result += ',';
            first = false;
            result += json_scalar_text(Value(key)) + ":" + information_value_signature(nested);
        }
        return result + "}";
    }
...
inline ptcg::ai::Value normalized_fingerprint_player(const ptcg::ai::Value &source) {
    ...
    if (bench.is_array()) {
        ...
        while (bench.as_array().size() < 5)
            bench.as_array().emplace_back();
        if (bench.as_array().size() > 5)
            bench.as_array().resize(5);
    }
    Value result(Value::Object{
        ...
        {"supporter_played_this_turn", copy_or("supporter_played_this_turn", Value(false))},
        {"energy_attached_this_turn", copy_or("energy_attached_this_turn", Value(false))},
        {"retreated_this_turn", copy_or("retreated_this_turn", Value(false))},
        {"stadium_played_this_turn", copy_or("stadium_played_this_turn", Value(false))},
        {"stadium_used_this_turn", copy_or("stadium_used_this_turn", Value(false))},
        {"healed_this_turn", copy_or("healed_this_turn", Value(false))},
...
inline std::string traditional_state_fingerprint(const ptcg::ai::RulesSession &position) {
    ...
    ptcg::ai::Value players =
        state.find("players") == nullptr ? ptcg::ai::Value::make_array() : *state.find("players");
    if (players.is_array()) {
        for (ptcg::ai::Value &player : players.as_array()) {
            player = normalized_fingerprint_player(player);
        }
    }
    ptcg::ai::Value rules_options =
        state.find("rules_options") != nullptr && state.find("rules_options")->is_object()
            ? *state.find("rules_options")
            : ptcg::ai::Value::make_object();
    const ptcg::ai::Value *matchups = state.find("apply_type_matchups");
    rules_options["apply_type_matchups"] =
        ptcg::ai::Value(matchups != nullptr && matchups->as_bool(false));
...
inline std::int64_t value_integer_field(const ptcg::ai::Value &value, const char *key,
                                        std::int64_t fallback = 0) {
    const ptcg::ai::Value *entry = value.find(key);
    return entry == nullptr ? fallback : entry->as_integer(fallback);
}

inline const ptcg::ai::Value &value_player(const ptcg::ai::Value &state, std::int32_t actor) {
    ...
    return players != nullptr && players->is_array() && actor >= 0 &&
               ...
               ? players->as_array()[static_cast<std::size_t>(actor)]
               : empty;
}

inline const ptcg::ai::Value *value_pokemon_at(const ptcg::ai::Value &state, std::int32_t actor,
    ...
    if (parsed.ec != std::errc{} || parsed.ptr != suffix.data() + suffix.size() ||
        bench == nullptr || !bench->is_array() || index >= bench->as_array().size())
        return nullptr;
    const ptcg::ai::Value &pokemon = bench->as_array()[index];
    return pokemon.is_object() ? &pokemon : nullptr;
}

inline const ptcg::ai::Value &value_catalog_cards(const ptcg::ai::Value &catalog) {
    const ptcg::ai::Value *cards = catalog.find("cards");
    return cards != nullptr && cards->is_object() ? *cards : catalog;
}

inline const ptcg::ai::Value *value_card(const ptcg::ai::Value &catalog,
...
inline std::string value_action_signature(const ptcg::ai::Value &action) {
    const ptcg::ai::TraditionalStableSignature stable = [](const ptcg::ai::Value &value) {
        return stable_value_signature(value);
    };
    const std::function<std::string(const std::string &)> sha = [](const std::string &value) {
...
inline ptcg::ai::Value action_intent(const ptcg::ai::Value &action,
    ...
    Value result(Value::Object{
        ...
        {"payload", payload != nullptr && payload->is_object() ? *payload : Value::make_object()},
    });
    ...
    result["signature"] = Value("intent:" + sha256_text(stable_value_signature(stable)));
...
inline std::string traditional_decision_semantic_hash(const ptcg::ai::Value &selected,
    ...
    if (preconditions.is_array() && preconditions.as_array().empty() && turn_plan.is_array()) {
    ...
    Value payload(Value::Object{
        ...
        {"engine_id", copy_or("engine_id", Value("deck_planner_v1"))},
...
inline bool value_ability_is_repeatable(const ptcg::ai::Value &state, std::int32_t actor,
                                        const ptcg::ai::Value &action,
                                        const std::string &ability_name,
                                        const ptcg::ai::Value &catalog) {
    const ptcg::ai::Value *pokemon = value_pokemon_at(state, actor, action_primary_slot(action));
    if (pokemon == nullptr)
        return false;
    const ptcg::ai::Value *definition =
        value_card(catalog, value_string_field(*pokemon, "card_id"));
    const ptcg::ai::Value *abilities =
        definition == nullptr ? nullptr : definition->find("abilities");
    ...
    for (const ptcg::ai::Value &ability : abilities->as_array()) {
        if (value_string_field(ability, "name") == ability_name &&
            value_string_field(ability, "trigger") == "repeatable")
            return true;
    }
    return false;
}

inline std::size_t repeatable_ability_uses_this_turn(const ptcg::ai::Value &state,
                                                     std::int32_t actor,
                                                     const std::string &ability_name) {
    const std::string player_name = value_string_field(value_player(state, actor), "name");
...
inline ptcg::ai::Value filter_exhausted_repeatable_abilities(const ptcg::ai::Value &state,
                                                             std::int32_t actor,
                                                             const ptcg::ai::Value &actions,
                                                             const ptcg::ai::Value &catalog) {
    if (!actions.is_array())
        return ptcg::ai::Value::make_array();
    ...
    for (const ptcg::ai::Value &action : actions.as_array()) {
        ...
        const std::string ability_name =
            payload == nullptr ? std::string{} : value_string_field(*payload, "ability_name");
        if (ability_name.empty() ||
            !value_ability_is_repeatable(state, actor, action, ability_name, catalog) ||
            repeatable_ability_uses_this_turn(state, actor, ability_name) < 6) {
            kept.push_back(action);
        }
    }
    if (!kept.empty())
        return ptcg::ai::Value(std::move(kept));
    if (!terminal.empty())
        return ptcg::ai::Value(std::move(terminal));
    return actions;
}

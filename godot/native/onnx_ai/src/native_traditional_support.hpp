...
namespace godot::native_traditional {

inline Dictionary dictionary_from_value(const ptcg::ai::Value &value) {
...
inline std::string godot_string_utf8(const String &value) {
...
inline String stable_value_signature(const ptcg::ai::Value &value) {
    ...
    switch (value.type()) {
        case Type::object: {
            ...
            for (const auto &[key, nested] : value.as_object()) {
                parts.append(
                    String::utf8(key.c_str()) + String("=")
                    + stable_value_signature(nested));
            }
        ...
        case Type::array: {
            ...
            for (const ptcg::ai::Value &nested : value.as_array()) {
                parts.append(stable_value_signature(nested));
            }
            return String("[") + String(",").join(parts) + String("]");
        }
        default:
            return JSON::stringify(ptcg::ai::value_to_godot(value));
    }
}

inline String information_value_signature(const ptcg::ai::Value &value) {
    using Type = ptcg::ai::Value::Type;
    if (value.type() == Type::object) {
        PackedStringArray parts;
        for (const auto &[key, nested] : value.as_object()) {
            parts.append(
                JSON::stringify(String::utf8(key.c_str())) + String(":")
    ...
    if (value.type() == Type::array) {
        PackedStringArray parts;
        for (const ptcg::ai::Value &nested : value.as_array()) {
            parts.append(information_value_signature(nested));
        }
        return String("[") + String(",").join(parts) + String("]");
    }
    return JSON::stringify(ptcg::ai::value_to_godot(value));
...
inline String traditional_state_fingerprint(const ptcg::ai::RulesSession &position) {
    ...
    return stable_value_signature(payload).sha256_text();
}
...
inline std::string value_action_signature(const ptcg::ai::Value &action) {
    ...
    const std::function<std::string(const std::string &)> sha = [](
        ...
        return godot_string_utf8(String::utf8(value.c_str()).sha256_text());
    };
...
inline ptcg::ai::Value action_intent(
    ...
    result["signature"] = Value("intent:" + godot_string_utf8(
        stable_value_signature(stable).sha256_text()));
...
inline std::string traditional_decision_semantic_hash(
    ...
    return godot_string_utf8(stable_value_signature(payload).sha256_text());
}
...
inline std::string action_cycle_state_fingerprint(ptcg::ai::Value state) {
    ...
    return godot_string_utf8(stable_value_signature(state).sha256_text());
}

...
func _show_card_art(context: Dictionary, return_action: Callable, return_label: String,
	...
	host.open(str(card.get("name", "卡牌")) + " · 卡图", "返回卡牌详情", "", in_battle, spec)
...
func _show_zone_card_inspector(
	...
	_show_card_inspector(
		card_context,
		_restore_zone_inspector.bind(zone_context, field_choice_context, host.modal_scroll.scroll_vertical),
		"返回区域查看",
		field_choice_context,
	)

func _restore_zone_inspector(context: Dictionary, choice_context: Dictionary, scroll: int) -> void:
	_show_zone_inspector(context, choice_context)
	_restore_modal_scroll(host.generation, scroll)

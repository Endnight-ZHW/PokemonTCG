...
func _update_energy_action_buttons(selected_count: int) -> void:
	...
	if clear_button:
		clear_button.disabled = not _energy_distribution_mode or selected_count <= 0
...
func _update_selection_hint(selected_count: int) -> void:
	...
	if _selection_max <= 0:
		selection_hint_label.visible = false
		return
	if _energy_distribution_mode or _request_type == "distribute_energy":
		_update_energy_selection_hint(selected_count)
		return
...
func _update_energy_selection_hint(selected_count: int) -> void:
	var minimum := _effective_min_select()
	if selected_count >= _selection_max:
		selection_hint_label.text = "已完成 %d / %d 张能量分配 · 可以确认" % [
			selected_count,
			_selection_max,
		]
		...
		selection_hint_label.text = (
			...
			% [selected_count + 1, _selection_max]
		)
	else:
		selection_hint_label.text = "正在分配第 %d / %d 张能量 · 还需分配 %d 张" % [
			selected_count + 1,
			_selection_max,
			minimum - selected_count,
		]

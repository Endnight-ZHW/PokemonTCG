...
class ActionResolver:
    ...
    def _declare_attack(self, player_idx: int, attack_idx: int) -> ActionResult:
        ...
        # Apply base damage via event-driven pipeline
        if attack.damage > 0 and defender is not None and not result.attack_failed:
            if defender.damage_prevented_next_turn:
                ...
                dmg_msg = (f"对{defender.card.name}造成了{final_damage}点伤害"
                           f"（剩余HP {defender.current_hp}）。")
                self.state._log(dmg_msg)
        ...
        # Check win condition immediately after KOs (e.g. last prize taken)

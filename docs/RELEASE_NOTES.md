...
- 修复原生迁移后传统 AI 把脱敏观察误当 Snapshot 3 恢复、继而持续落入单步 fallback
  的回归。主场景、评测器和 Deep→Challenge 现共用 `AIRuntimeStateProjection`；动作请求
  必须携带权威 Action v4 集合，并严格拒绝过期 revision、重复动作、隐藏引用及私有状态。
- 原生规则会话新增确定性 `fork_for_search(seed)` 与合并 fork/apply 绑定；搜索分支共享只读
  目录/kernel、不写 journal，GameEngine 以实例/revision/内容指纹管理决策级会话。普通恢复
  路径延迟构造 DTO，冻结 v1 在不改预算/阈值下恢复校准工作量。
- 修复击倒后选择奖励牌期间打开弃牌区、卡牌详情、对局菜单、帮助或游戏内设置会丢失
  `ChoiceView` 并卡住对局的问题。辅助弹窗现在显式暂停场上 Choice，关闭按钮、系统返回和
  嵌套查看均恢复当前权威请求；网络 revision 已变化时不会复活过期选择。
- 弃牌区卡牌来源现进入统一动作路由；空手牌时可以通过己方弃牌区的独立“可用操作”按钮
  发动帝王拿波的「紧急上浮」，点击牌堆主体仍可正常查看弃牌区。同类公开区域来源动作
  也不再被 UI 判为不可达。显式卡牌移动事件优先于
  结算前后净差：使用/弃置的卡与新抽卡同名时，弃牌与抽牌仍分别播放完整物理动画。
